...
SIGNIN_RESPONSE=$(curl -s --fail-with-body "$API_URL/authentication/sign-in" \
  ...
  -d '{"email":"bench@aiqadam.org","password":"BenchmarkPass1"}')
...
for i in $(seq 1 300); do
  HAS_DELAY=$(curl -sf "$API_URL/qadams" 2>/dev/null | jq '[.[].name] | any(. == "@aiqadam/qadam-delay")' 2>/dev/null || echo "false")
