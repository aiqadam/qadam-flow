...
description: "Configure SCIM provisioning from Okta to Qadam Flow."
---
...
- Generated an API key from `/platform/security/api-keys` route in the Qadam Flow app

## Configure SCIM Connection in Okta

<Steps>
  <Step title="Open your Okta app">
    In Okta Admin Console, open your Qadam Flow application (created in first [SSO step](/admin-guide/guides/sso#saml-with-okta)).
  ...
  <Step title="Add SCIM connection details">
    **Provisioning** will be visible, go to it and set:
    - **SCIM base URL** to `https://your-qadam-flow-domain/api/v1/scim/v2`
...
<Steps>
  ...
  <Step>
    ...
    <Warning>
      This step assumes that you already have a field in the Okta user profile that you can map to platformRole in your Qadam Flow user profile. If you don't have one, you can create a new field in **Directory -> Profile Editor -> User (default)**.
    </Warning>

     - Back to your Qadam Flow application page in **Provisioning -> To App -> Attribute Mappings**
...
In the Qadam Flow application page, go to **Provisioning -> To App** and enable the actions you want to be applied to Qadam Flow when changes occur in Okta.

![Okta role expression example](/resources/screenshots/scim/okta/provision-actions.png)

Now in the **Assignments** tab you can:
- Choose to provision individual users or groups. Note that groups in Okta will be projects in Qadam Flow.

- In case you don't have groups and you want to provision your Okta users at once, you can assign the `Everyone` group.

- When editing/creating users in an assigned group (including `Everyone`), they should be updated in Qadam Flow.

- To push groups to Qadam Flow, go to the **Push groups** tab and click on the push button, find the group and save.
...
<Tip>
  Created users in Qadam Flow will receive a welcome email. When clicked, they will be redirected to sign in with `SAML`.
</Tip>


### Deprovision

Users' state switches to `INACTIVE` in Qadam Flow only when they are deactivated in Okta. Suspension or deletion in Okta does not reflect in Qadam Flow because of Okta's design.
...
<Warning>
  Deleting a group will delete the whole project in Qadam Flow with its flows and connections. Users linked to that group won't be affected.
</Warning>
