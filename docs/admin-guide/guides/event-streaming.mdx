...
Event Streaming forwards platform [audit events](/admin-guide/security/audit-logs/overview) to a webhook URL of your choice. The most common pattern is to point that webhook at a flow inside Qadam Flow — your flow then routes each event to the channel you care about (Slack, Gmail, Microsoft Teams, custom HTTP, etc.).
...
4. Customize the branches with whatever qadams you like — Slack, Gmail, Microsoft Teams, custom HTTP, and so on.
