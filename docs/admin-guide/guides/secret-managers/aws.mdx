...
description: "Connect AWS Secrets Manager to Qadam Flow for centralized secret management"
---
...
Create an IAM policy that grants read access to the secrets Qadam Flow will retrieve.
...
<Tip>
For production, scope `Resource` to the specific secret ARNs Qadam Flow needs instead of using `"*"`.
</Tip>

3. Click **Next**, name the policy (e.g. `Qadam FlowSecretsReadOnly`), and create it.
...
Qadam Flow will use the configured credentials to retrieve the secret value and inject it into the connection at runtime.
