...
description: "Connect CyberArk Conjur to Qadam Flow for centralized secret management"
---

CyberArk Conjur is a secrets management solution that provides secure storage and access to credentials. Integration with Qadam Flow uses **host/API key authentication**: Qadam Flow authenticates as a Conjur host, receives a short-lived token, and uses it to retrieve secrets for which that host has `read` and `execute` permissions.
...
- A Conjur policy that defines a host for Qadam Flow and grants it access to the variables you want to use

## Conjur host configuration for Qadam Flow

To allow Qadam Flow to read secrets, configure a Conjur policy that declares a group, variables, a host, a layer, and the right permissions. The steps below describe how to create that policy file.

### Example policy (Qadam Flow)

The following policy defines a policy `Qadam Flow` with a group, two variables, a host, a layer, and the grants so the host can read the variables.
...
- !policy
  ...
  body:
    ...
    - !layer Qadam Flow
    - !grant
        role: !layer Qadam Flow
        members:
            - !host Qadam Flow
    - !grant
        role: !group activepieces-secrets
        member: !layer Qadam Flow
```
...
3. **Declare the host** that Qadam Flow will use (e.g. `Qadam Flow`) and a **layer** (e.g. `Qadam Flow`), and add the host to the layer:
...
5. **Load the policy** into Conjur. Conjur will create the host and return an **API key** for that host. You will use this API key and the host identity when connecting Qadam Flow.
...
Store the **api_key** securely; you will enter it in Qadam Flow as the **API Key**.
...
Qadam Flow will authenticate as the configured host and retrieve the secret from Conjur when the flow runs.
