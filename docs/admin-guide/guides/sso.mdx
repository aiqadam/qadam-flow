...
description: "Configure Single Sign-On (SSO) to enable secure, centralized authentication for your Qadam Flow platform"
icon: 'key'
---
...
Single Sign-On (SSO) allows your team to authenticate using your organization's existing identity provider, eliminating the need for separate Qadam Flow credentials. This improves security, simplifies user management, and provides a seamless login experience.

## Prerequisites

Before configuring SSO, ensure you have:

- **Admin access** to your Qadam Flow platform
- **Admin access** to your identity provider (Google, GitHub, Okta, or JumpCloud)
- The **redirect URL** from your Qadam Flow SSO configuration screen

## Accessing SSO Configuration

Navigate to **Platform Settings** → **SSO** in your Qadam Flow admin dashboard to access the SSO configuration screen.
...
Qadam Flow supports multiple SSO providers to integrate with your existing identity management system.
...
<Steps>
    ...
    <Step title="Configure Redirect URI">
        Copy the **Redirect URL** from the Qadam Flow SSO configuration screen and add it to the **Authorized redirect URIs** in Google Cloud Console.
    </Step>
    <Step title="Copy Credentials to Qadam Flow">
        Copy the **Client ID** and **Client Secret** from Google and paste them into the corresponding fields in Qadam Flow.
    </Step>
...
<Steps>
    ...
    <Step title="Register New Application">
        ...
        - **Application name**: Choose a recognizable name (e.g., "Qadam Flow SSO")
        - **Homepage URL**: Enter your Qadam Flow instance URL
    </Step>
    <Step title="Configure Authorization Callback">
        Copy the **Redirect URL** from the Qadam Flow SSO configuration screen and paste it into the **Authorization callback URL** field.
    </Step>
    ...
    <Step title="Copy Credentials to Qadam Flow">
        Copy the **Client ID** and **Client Secret** and paste them into the corresponding fields in Qadam Flow.
    </Step>
    <Step title="Save Configuration">
        Click **Finish** to complete the setup.
    </Step>
...
<Steps>
    ...
    <Step title="Configure General Settings">
        Enter an **App name** (e.g., "Qadam Flow") and optionally upload a logo. Click **Next**.
    </Step>
    <Step title="Configure SAML Settings">
        - **Single sign-on URL**: Copy the SSO URL from the Qadam Flow configuration screen
        - **Audience URI (SP Entity ID)**: Enter `Qadam Flow`
    ...
    <Step title="Configure Qadam Flow">
...
<Steps>
    <Step title="Create an Enterprise Application">
        ...
        Name it (e.g., "Qadam Flow") and select **Integrate any other application you don't find in the gallery (Non-gallery)**.
    </Step>
    ...
    <Step title="Set Identifier and Reply URL">
        ...
        - **Identifier (Entity ID)**: `Qadam Flow`
        - **Reply URL (Assertion Consumer Service URL)**: paste the SSO URL from the Qadam Flow configuration screen
    </Step>
    ...
    <Step title="Copy the Federation Metadata">
        ...
        You can paste this URL directly into the **IdP Metadata** field in Qadam Flow — Qadam Flow will fetch the metadata XML automatically. Alternatively, open the URL in a browser, save the XML, and paste its contents.
    </Step>
    <Step title="Copy the Signing Certificate">
        Download the **Certificate (Base64)** from the **SAML Certificates** section. Open the file and copy its contents (including the `-----BEGIN CERTIFICATE-----` / `-----END CERTIFICATE-----` markers) into the **Signing Key** field in Qadam Flow.
    </Step>
    <Step title="Assign Users">
        Go to **Users and groups** in the application and assign the users or groups that should be allowed to sign in.
    </Step>
    <Step title="Save Configuration">
        Click **Save** in Qadam Flow to complete the setup.
    </Step>
</Steps>
...
<Steps>
    ...
    <Step title="Configure ACS URL">
        Copy the **ACS URL** from the Qadam Flow configuration screen and paste it into the **ACS URLs** field in JumpCloud.
        
        ![JumpCloud ACS URL](/resources/screenshots/jumpcloud/acl-url.png)
    </Step>
    <Step title="Configure SP Entity ID">
        Set the **SP Entity ID** (Audience URI) to `Qadam Flow`.
    </Step>
    ...
    <Step title="Configure IdP Metadata in Qadam Flow">
        Paste the exported metadata XML into the **IdP Metadata** field in Qadam Flow.
    </Step>
...
<AccordionGroup>
    ...
    <Accordion title="SAML authentication fails">
        ...
        - If you pasted a metadata URL, make sure it is publicly reachable (Qadam Flow fetches it server-side)
...
Open an issue on [GitHub](https://github.com/aiqadam/qadam-flow/issues) or join the AI Qadam community channels.
