...
By default, navigating within your embedded instance of Qadam Flow doesn't affect the client's browser history or viewed URL. Qadam Flow only provide a **handler**, that trigger on every route change in the **iframe**.

## Automatically Sync URL 

You can use the following snippet when configuring the SDK, which will implement a handler that syncs the Qadam Flow iframe with your browser:
...
window.addEventListener("popstate", () => {
  const route = activepieces.extractQadam FlowRouteFromUrl({ vendorUrl: window.location.href });
