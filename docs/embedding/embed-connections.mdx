...
<Info>
**Requirements:**
* Qadam Flow version 0.34.5 or higher
...
<Warning>
**Breaking Change:** <br/><br/> If your Qadam Flow instance version is < 0.45.0 and (you are using the connect method from the embed sdk, and need the connection externalId to be returned after the user creates it OR if you want to reconnect a specific connection with an externalId), you must upgrade your instance to >= 0.45.0
</Warning>

- You can use the embedded SDK in your SaaS to allow your users to create connections and store them in Qadam Flow.
...
<Steps>
    ...
    <Step title="Call Connect Method">
        ...
        ```html
        <script> 
        activepieces.connect({pieceName:'@aiqadam/qadam-google-sheets'});
        </script>
        ```
        ...
        **Connect Parameters:**
        ...
        | -------------- | -------- | ------ | ------------------------------------------- |
        | pieceName      | ✅      | string | The name of the qadam you want to create a connection for.
        ...
        **Connect Result**
        ...
        <Tip>
        You can use the `connections` qadam in the builder to retrieve the created connection using its name.
        ![Connections in Builder](/resources/screenshots/connections-qadam.png)
        ![Connections in Builder](/resources/screenshots/connections-qadam-usage.png)
        </Tip>
    </Step>
</Steps>
