...
description: "Automatically authenticate your SaaS users to your Qadam Flow instance"
icon: 'user'
---
...
In Qadam Flow, there are **Projects** and **Users**. Each project is provisioned with their corresponding workspace, project, or team in your SaaS. The users are then mapped to the respective users in Qadam Flow.
...
<Steps>
  <Step title="Step 1: Obtain Signing Key">
    ...
    This will generate a public and private key pair. The public key will be used by Qadam Flow to verify the signature of the JWT tokens you send. The private key will be used by you to sign the JWT tokens.

    ![Create Signing Key](../resources/screenshots/create-signing-key.png)
    <Warning>
      Please store your private key in a safe place, as it will not be stored in Qadam Flow.
    </Warning>
  </Step>

  <Step title="Step 2: Generate a JWT">
    The signing key will be used to generate JWT tokens for the currently logged-in user on your website, which will then be sent to the Qadam Flow Iframe as a query parameter to authenticate the user and exchange the token for a longer lived token.
    ...
    | role              | Role of the user in the Qadam Flow project (e.g., **EDITOR**, **VIEWER**, **ADMIN**)           |
    | exp               | Expiry timestamp for the token (Unix timestamp)  |
    | piecesFilterType  | Customize the project qadams, check [customize qadams](/embedding/customize-qadams)  |
    | piecesTags        | Customize the project qadams, check [customize qadams](/embedding/customize-qadams)  |
