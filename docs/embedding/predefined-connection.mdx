...
Use predefined connections to allow users to access your qadam in the embedded app without re-entering authentication credentials.
...
- (Optional) Hide the connections dropdown in the qadam settings.

### Prerequisites

- [Run the Enterprise Edition](/handbook/engineering/playbooks/run-ee)
- [Create your qadam](/build-qadams/building-qadams/overview). Later we will customize the qadam logic to use predefined connections.
...
const instanceUrl = 'https://qadam-flow.your-domain.com';

// The name of the user / organization in your SAAS
const externalProjectId = 'org_1234';
const pieceName = '@aiqadam/qadam-gelato';
// This will depend on what your qadam auth type is, can be one of this ['PLATFORM_OAUTH2','SECRET_TEXT','BASIC_AUTH','CUSTOM_AUTH']
const pieceAuthType = "CUSTOM_AUTH"
const connectionProps = {
    // Fill in the props required by your qadam's auth
}
...
<Steps>
    <Step title="Modify Trigger / Action Definition">
       Wherever you call `createTrigger` or `createAction` set `requireAuth` to `false`, this will hide the connections dropdown in the qadam settings in the builder,
