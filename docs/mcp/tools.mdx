...
icon: "wrench"
description: "Complete catalog of all MCP tools available in Qadam Flow"
---
...
Get the full structure of a flow: step tree, configuration status, step input values, router branch conditions, valid insert locations, and whether each step's pinned qadam version is still available on this installation. Shows the actual configured inputs for each step (URL, body, headers, etc.) so the AI can read and edit existing configurations.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `flowId` | string | Yes | The flow ID |
| `includeInput` | boolean | No | When true, return each step's full untruncated input in `structuredContent` and render text `input:` lines untruncated (default text output truncates long inputs) |
...
Research available qadams. Use `pieceNames` for bulk exact lookup (always returns actions and triggers). Use `searchQuery` for fuzzy discovery.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `pieceNames` | string[] | No | Exact qadam names to look up (always returns actions and triggers) |
| `searchQuery` | string | No | Filter qadams by name |
...
Get the detailed input property schema for a specific qadam action or trigger. Returns field names, types, required/optional, descriptions, default values, and dropdown options. When auth is required but not provided, automatically lists available connections. Use this before `ap_update_step` or `ap_update_trigger` to know exactly which fields to set.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `pieceName` | string | Yes | Qadam name (e.g., `@aiqadam/qadam-slack`) |
...
Resolve dropdown options for a single qadam property. Returns available choices with labels and internal IDs. Use this to discover valid values for dynamic dropdown fields (e.g., Slack channels, Google Sheets, email labels) before configuring a step.
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `pieceName` | string | Yes | Qadam name (e.g., `@aiqadam/qadam-google-sheets`) |
...
| `pieceName` | string | No | For PIECE types: qadam name (short names like `slack` accepted) |
...
| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `pieceName` | string | No | Filter by qadam name. Short names like `slack` or `google-sheets` are auto-expanded. |
...
| `filters` | array | No | Filter conditions (fieldName, operator, value, path) |
| `limit` | number | No | Max records (default 50, max 500) |

**Filter operators:** `eq`, `neq`, `gt`, `gte`, `lt`, `lte`, `co` (contains), `exists`, `not_exists`, `json_path_eq`

`json_path_eq` applies to a `JSON` column only and takes an extra `path` on the filter — a dot-separated path into the cell's parsed JSON, e.g. `{"fieldName": "Profile", "operator": "json_path_eq", "path": "address.city", "value": "Tashkent"}`. `gt`/`gte`/`lt`/`lte` are rejected on `BOOLEAN` and `JSON` columns, and `co` is rejected on `BOOLEAN`, because neither column type has a meaningful ordering.
...
| `pieceName` | string | No | For connections: which qadam needs auth |
...
|-------|------|----------|-------------|
| `flowId` | string | Yes | The flow ID |
| `pieceName` | string | Yes | Qadam name (e.g., `@aiqadam/qadam-gmail`) |
| `triggerName` | string | Yes | Trigger name from the qadam |
...
Cell values are strings on the wire for every column type. A `BOOLEAN` cell accepts `"true"`, `"false"`, or an empty string for unset — nothing else. A `JSON` cell accepts a serialised JSON document (`"{\"city\":\"Tashkent\"}"`), not a raw object; anything that does not parse is rejected at write time rather than stored and discovered later.
...
| `keyFields` | array | No | Field **names** (from `fields`) that together form the table's unique business key |

**Field types:** `TEXT`, `NUMBER`, `DATE`, `STATIC_DROPDOWN` (requires `options` array), `BOOLEAN`, `JSON`

`keyFields` names columns, not ids — the field ids do not exist yet when the table is created. Declaring a key on a brand-new table always succeeds, because there are no records to collide.
...
|-------|------|----------|-------------|
| `tableId` | string | Yes | The table ID |
| `operation` | string | Yes | `ADD`, `UPDATE`, `DELETE`, `DECLARE_KEY`, or `CLEAR_KEY` |
...
| `keyFieldIds` | array | No | Field **ids**, required for DECLARE_KEY |

`DECLARE_KEY` is rejected if the table already holds records that share the proposed key — clean up the duplicates first (`ap_find_records` / `ap_update_record` / `ap_delete_records`) and retry. A field that is part of the current key cannot be deleted until `CLEAR_KEY` runs.
...
Execute a single qadam action once without building or saving a flow. Designed for one-shot tasks like *"check my inbox"* or *"send one Slack message"* where building a full automation would be overkill. Under the hood the tool creates a disposable flow, runs the action, returns the output, and cleans the flow up — the user never sees it in their flow list.

| Input | Type | Required | Description |
|-------|------|----------|-------------|
| `pieceName` | string | Yes | Qadam name (e.g. `slack` or `@aiqadam/qadam-slack`). Use `ap_research_pieces` to discover. |
| `actionName` | string | Yes | Action to run (e.g. `send_channel_message`). Use `ap_get_piece_props` for the input shape. |
| `input` | object | No | Fully-resolved input for the action. Keys must match the qadam action's props. Pass raw values — do **not** wrap them in `{{…}}`. Omit entirely if the action has no props. |
| `connectionExternalId` | string | No | `externalId` from `ap_list_connections`. Required if the qadam needs auth. Auto-wrapped server-side as `{{connections['externalId']}}`. Must be a plain ID — special characters are rejected. |
