...
description: "How to rollback Qadam Flow to a previous version"
icon: "rotate-left"
---

## Overview

Qadam Flow ships a rollback command that reverses database migrations when you need to downgrade to a previous version. Most releases are fully rollback-safe — the release notes will let you know if one isn't.
...
dropdb Qadam Flow
createdb Qadam Flow
pg_restore -d Qadam Flow backup-pre-upgrade.dump
```

Then switch your `docker-compose.yml` to the previous image version and start Qadam Flow.
