...
icon: "server"
description: "Specifications for hosting Qadam Flow"
---
...
Qadam Flow is designed to be memory-intensive rather than CPU-intensive. A modest instance will suffice for most scenarios, but requirements can vary based on specific use cases.
...
Redis requires minimal scaling as it primarily stores jobs during processing. Qadam Flow leverages BullMQ, capable of handling a substantial number of jobs per second.
...
Qadam Flow ensures no request is lost; all requests are queued. In the event of a spike, requests will be processed later, which is acceptable as most flows are asynchronous, with synchronous flows being prioritized.
