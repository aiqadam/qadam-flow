...
- Setting only the old `AP_<NAME>` still works and behaves identically, but logs a one-time startup warning telling you to rename it to `QF_<NAME>` — **except for the ~10 names that `docker-entrypoint.sh`/`docker-compose.yml` read in the shell** (`AP_CONTAINER_TYPE`, `AP_PORT`, `AP_PM2_INSTANCES`, `AP_JWT_SECRET`, `AP_WORKER_TOKEN`, `AP_POSTGRES_DATABASE`, `AP_POSTGRES_PASSWORD`, `AP_POSTGRES_USERNAME`, `AP_QADAMS_SYNC_MODE`, `AP_WORKER_API_URL`) plus the already-deprecated `AP_*_PIECES` names. Those are exempt from the warning because renaming them today wouldn't do anything for the shell scripts that read them (see below) — warning about them would just tell every stock Docker install to make a change that breaks it.
