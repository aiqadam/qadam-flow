...
- Every environment variable that was previously read as `AP_<NAME>` (e.g. `AP_JWT_SECRET`, `AP_POSTGRES_DATABASE`, `AP_CONTAINER_TYPE`) can now also be set as `QF_<NAME>` (e.g. `QF_JWT_SECRET`).
- If both `QF_<NAME>` and `AP_<NAME>` are set, `QF_<NAME>` wins.
...
- The Docker Compose file and `run.sh` install script still ship with `AP_*` names for now; that migration is tracked separately so operators aren't asked to change working `.env` files before the alias layer has a release behind it.
