...
- Every environment variable that was previously read as `AP_<NAME>` (e.g. `AP_JWT_SECRET`, `AP_POSTGRES_DATABASE`, `AP_CONTAINER_TYPE`) can now also be set as `QF_<NAME>` (e.g. `QF_JWT_SECRET`) **when it reaches the Node server or worker process** — directly, via `.env`, or via `docker run -e`.
