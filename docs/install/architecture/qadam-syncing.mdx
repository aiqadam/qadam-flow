...
- **Official qadams ship bundled with the server** — each release bundles a fixed set of official qadams (`loadBundledQadams`); there is no cloud registry to sync from, so picking up new official qadams means upgrading the server.
- **Each step is pinned to an exact version** — flows never auto-upgrade. Bumps are explicit, through the builder.

```mermaid
flowchart LR
    BUNDLE["Bundled qadams<br/>(shipped with release)"]
    CAT["Local qadam catalog"]
    STEP["Flow step<br/>pieceVersion: 0.5.3"]
    UI["Builder version dialog"]

    BUNDLE -- "loaded on startup" --> CAT
...
|---|---|---|
| Official | Bundled with the Qadam Flow release | Loaded automatically on startup |
