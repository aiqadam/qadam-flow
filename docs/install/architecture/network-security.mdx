...
description: "How Qadam Flow isolates outbound traffic from your internal network"
icon: "shield-halved"
---

## Overview

Qadam Flow makes outbound HTTP from two surfaces, and each is hardened separately:

1. **User code in flows** — Code steps, qadam actions, anything a flow author can write. Hardened by `AP_NETWORK_MODE`; see [User-code egress](#user-code-egress).
...
```mermaid
flowchart LR
    UC["User code<br/>(Code step · Qadam · fetch)"]
