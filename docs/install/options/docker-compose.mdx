...
**1. Clone Qadam Flow repository.**

Use the command line to clone Qadam Flow repository:
...
```bash
docker compose -p Qadam Flow up
```
...
To upgrade to new versions, which are installed using docker compose, perform the following steps. First, open a terminal in the Qadam Flow repository directory and run the following commands.
