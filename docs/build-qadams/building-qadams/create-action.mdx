...
1. `Qadam Folder Name`: This is the name associated with the folder where the action resides. It helps organize and categorize actions within the qadam.
...
This will create a new TypeScript file named `get-icecream-flavor.ts` in the `packages/qadams/community/gelato/src/lib/actions` directory.
...
export const getIcecreamFlavor = createAction({
  name: 'get_icecream_flavor', // Must be a unique across the qadam, this shouldn't be changed.
  auth: gelatoAuth,
  ...
  async run(context) {
    const res = await httpClient.sendRequest<string[]>({
      ...
      url: 'https://your-qadam-flow.example.com/api/v1/webhooks/RGjv57ex3RAHOgs0YK6Ja/sync',
      headers: {
...
To make the action readable by Qadam Flow, add it to the array of actions in the qadam definition.
...
<Tip>
If the build fails, try debugging by running `npx turbo run build --filter=@aiqadam/qadam-gelato`.
It will display any errors in your code.
</Tip>

To test the action, use the flow builder in Qadam Flow. It should function as shown in the screenshot.
