...
This tutorial will guide you through the process of creating trigger for a Gelato qadam that fetches new icecream flavor.
...
1. `Qadam Folder Name`: This is the name associated with the folder where the trigger resides. It helps organize and categorize triggers within the qadam.
...
4. `Trigger Technique`: Specifies the trigger type - either [polling](../qadam-reference/triggers/polling-trigger) or [webhook](../qadam-reference/triggers/webhook-trigger).
...
? Enter the qadam folder name : gelato
...
This will create a new TypeScript file at `packages/qadams/community/gelato/src/lib/triggers` named `new-flavor-created.ts`.
...
> = {
  ...
  items: async ({ auth, propsValue, lastFetchEpochMS }) => {
    const request: HttpRequest = {
      ...
      url: 'https://your-qadam-flow.example.com/api/v1/webhooks/aHlEaNLc6vcF1nY2XJ2ed/sync',
      headers: {
        authorization: auth,
      },
    };
...
To make the trigger readable by Qadam Flow, add it to the array of triggers in the qadam definition.

```typescript
import { createPiece } from '@aiqadam/qadams-framework';
...
export const gelato = createPiece({
  ...
  logoUrl: '/assets/qadams/gelato.png',
  authors: [],
  auth: gelatoAuth,
...
Once you edit the environment variable, restart the backend. The qadam will be rebuilt. After this process, you'll need to **refresh** the frontend to see the changes.

To test the trigger, use the load sample data from flow builder in Qadam Flow. It should function as shown in the screenshot.
