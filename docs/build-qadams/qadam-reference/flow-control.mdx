...
title: 'Flow Control'
icon: 'Joystick'
description: 'Learn how to control flow execution from inside a qadam'
---
...
<Warning>
  **Deprecated:** older qadams use `ctx.run.pause({ pauseMetadata: { type: PauseType.WEBHOOK | PauseType.DELAY, ... } })` together with `ctx.generateResumeUrl(...)`. That V0 API is kept for backwards compatibility with in-flight paused runs and will be removed. New actions must use `ctx.run.createWaitpoint` + `ctx.run.waitForWaitpoint`.
</Warning>
