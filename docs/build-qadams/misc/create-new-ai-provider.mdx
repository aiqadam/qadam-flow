...
Qadam Flow currently supports the following AI providers:
...
Create a new factory that returns an instance of the `AI` interface in the `packages/qadams/common/src/lib/ai/providers/your-ai-provider.ts` file.
...
Add the new AI provider to the `AiProviders` enum in `packages/qadams/common/src/lib/ai/providers/index.ts` file.
...
Now we need to tell Qadam Flow how to authenticate to your AI provider. You can do this by adding an `auth` property to the `AiProvider` object.
...
To test the AI provider, you can use a **universal AI** qadam in a flow. Follow these steps:
...
- Create a flow that uses our **universal AI** qadams. And select **"Your AI Provider"** as the AI provider in the **Ask AI** action settings.
