...
- **Location:** `packages/qadams/community/` by default; `packages/qadams/custom/` only if the user says "custom piece". See Piece Types below.
...
Create this structure under `packages/qadams/community/<name>/`:
...
**When you need a pattern, read the relevant reference file — do not grep other pieces in the codebase.** The reference files contain copy-ready examples for every common case. Searching `packages/qadams/community/` surfaces inconsistent older code and wastes context.
...
- [ ] Register in `tsconfig.base.json` at repo root (insert **alphabetically** — build fails without this):
    ```json
    "@aiqadam/qadam-<name>": ["packages/qadams/community/<name>/src/index.ts"]
    ```
...
npx turbo run build --filter=@aiqadam/qadam-<name>
npx turbo run lint --filter=@aiqadam/qadam-<name>
```
...
A flow built against the old schema stores its configuration once, at authoring time. If you add a **required** prop with no `defaultValue`, every flow that was correctly configured before your change now has a `step.settings.input` with no value for a field the schema demands — it was never wrong, and nobody touched it.
...
| Location | Purpose |
|---|---|
| `packages/qadams/community/` | Third-party integrations (Slack, Stripe, etc.) — use this for almost all work |
| `packages/qadams/core/` | Built-in platform utilities (HTTP, Store, Math, etc.) — do NOT recreate these |
| `packages/qadams/custom/` | Private customer-specific pieces |

Full reference: [piece-types.md](piece-types.md) — includes all `QadamCategory` values and the list of existing core pieces.
...
| Username + password | `QadamAuth.BasicAuth()` | `context.auth.username`, `context.auth.password` |
| Multiple fields | `QadamAuth.CustomAuth()` | `context.auth.props.<field_name>` |
| No auth needed | `QadamAuth.None()` | No `context.auth` available |
...
export const myAppAuth = QadamAuth.SecretText({
    displayName: 'API Key',
