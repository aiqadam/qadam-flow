...
Use when the API does NOT support webhooks. Qadam Flow polls every ~5 minutes.
...
import { DedupeStrategy, Polling, pollingHelper, httpClient, HttpMethod, AuthenticationType } from '@aiqadam/qadams-common';
...
**Real example:** `packages/qadams/community/airtable/src/lib/trigger/new-record.trigger.ts`
...
import { httpClient, HttpMethod, AuthenticationType } from '@aiqadam/qadams-common';
import { myAppAuth } from '../../';
...
export const newRecordWebhookTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    const response = await httpClient.sendRequest<{ id: string }>({
      ...
      body: {
        url: context.webhookUrl,         // Qadam Flow provides this
...
**Real example:** `packages/qadams/community/stripe/src/lib/trigger/new-customer.ts`
...
```typescript
import { WebhookHandshakeStrategy } from '@aiqadam/qadams-framework';
