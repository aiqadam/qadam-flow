...
**Real example:** `packages/qadams/community/github/src/lib/common/index.ts` -- see `repositoryDropdown`, `issueDropdown`, `labelDropDown`
...
For the `QadamCategory` values used in `createQadam({ categories: [...] })`, see `piece-types.md` — it lists every category with guidance on which to pick. (Categories aren't a prop type, so they live there rather than here.)
