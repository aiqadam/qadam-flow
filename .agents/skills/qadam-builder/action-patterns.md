...
The `token` field above assumes a `QadamAuth.SecretText()` auth. For other auth types, swap to `context.auth.access_token` (OAuth2), `context.auth.username`/`.password` (BasicAuth), or `context.auth.props.<field>` (CustomAuth). See `auth-patterns.md` for the full table.

**Real example:** `packages/qadams/community/github/src/lib/actions/create-issue.ts`
