...
**fileService**
- `save({ fileId?, projectId, platformId?, data, size, type, fileName?, compression, metadata? })` — determines location based on type, saves to DB or S3 (with DB fallback on S3 error). Returns the persisted `File` entity. The write is an atomic `INSERT ... ON CONFLICT (id) DO UPDATE ... WHERE` ownership check (`upsertOwnedFile` in `file.service.ts`): a caller-chosen `fileId` (e.g. the engine's `PUT /v1/files/:fileId`) can only create a new row or update one it already owns — a project-owned row matches by `projectId` alone (platformId is backfilled via `COALESCE` rather than required to already match, since some callers never pass it), a platform-level row (`projectId` null) requires an exact `platformId` match via `IS NOT DISTINCT FROM`. An id already owned by someone else throws `ErrorCode.AUTHORIZATION` instead of taking the row over (#517). For the S3 location, this upsert runs *before* the upload: the S3 key template (`platform/${platformId}/${type}/${fileId}`) ignores `projectId`, so two projects on the same platform can compute the identical key for the same `fileId` — the ownership check, not key scoping, is what stops one project's upload from landing on (or overwriting) another's object.
