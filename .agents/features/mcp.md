...
- **MCP trigger piece** — `@aiqadam/qadam-mcp`; a flow with this trigger is exposed as a callable tool via MCP
...
- `ap_list_translations`, `ap_upsert_translations`, `ap_delete_translation` — project translation management (`{{$t['key']}}` mentions); see `translations.md`. `ap_upsert_translations` is a batch upsert-by-key that merges into existing locales, same as its REST endpoint.

**Chat gating.** Every tool registered here is also reachable from the chat agent, where `chat-tool-gating.ts` is **default-deny**: a tool nobody listed requires user approval per call. Adding a tool therefore means deciding its group there and updating the counted assertions in `test/unit/app/chat/chat-tool-gating.test.ts` — that test exists precisely so a new tool cannot join the ungated set silently. `ap_export_flow` / `ap_export_table` / `ap_list_variables` / `ap_list_translations` are in the read-only group (they write nothing): the two exports and `ap_list_variables` because their data is already reachable via the ungated `ap_flow_structure` / `ap_find_records`, or (for variables) because no secret value is returned at all; `ap_list_translations` on the different grounds that a translation value is simply not a secret, not because it is reachable elsewhere (it isn't — `ap_flow_structure` shows only the `$t[...]` key reference, never the resolved value). The four import/variable writers and the two translation writers are gated.
