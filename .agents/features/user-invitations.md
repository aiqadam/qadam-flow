...
- Deleting a PLATFORM invitation runs `assertPrincipalIsPlatformAdmin`, matching create: a USER principal must have `PlatformRole.ADMIN` (OPERATOR is rejected, same as create), SERVICE principals pass. Fixed — previously the handler only asserted when `invitation.type === InvitationType.PROJECT`, leaving PLATFORM deletes fully unchecked.
