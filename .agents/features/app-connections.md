...
## Summary
App Connections store encrypted authentication credentials (OAuth2 tokens, API keys, basic auth, or custom qadam-defined fields) that flow steps use to call external services. They support automatic OAuth2 token refresh with distributed locking, a dual-scope model (project-level or platform-wide), and a "replace" operation that atomically rewires all flow references from one connection to another. The module handles two OAuth2 variants: user-supplied credentials and platform-managed OAuth apps. Users can optionally select a subset of a qadam's declared OAuth2 scopes when creating a connection.
...
- **AppConnectionType**: One of `OAUTH2`, `PLATFORM_OAUTH2`, `SECRET_TEXT`, `BASIC_AUTH`, `CUSTOM_AUTH`, `NO_AUTH`.
