...
## Summary
A built-in relational database feature that lets users store structured data directly within Qadam Flow, without needing an external database. Tables support typed fields, cell-level storage, per-row webhooks that fire flow automations, and a rich spreadsheet-like editor in the UI. They are tightly integrated with the flow engine through the Tables piece, which provides trigger and action steps for reacting to and manipulating table data.

## Key Files
- `packages/server/api/src/app/tables/table/table.service.ts` — table CRUD, export, webhook management
- `packages/server/api/src/app/tables/table-import.service.ts` — shared table-import logic used by `ap_import_table` (`create` and `into-existing` modes). The `into-existing` clear-and-recreate-schema sequence (delete records → delete fields → rename → recreate fields) runs inside one DB transaction via an optional `entityManager` threaded through `fieldService`/`tableService`/`recordService.deleteAll`, so a mid-sequence failure rolls back instead of leaving the table wiped and fieldless. Two consequences of that transaction are load-bearing: every field insert inside it would otherwise share one `now()` default (the *transaction* timestamp) while `fieldService.getAll` orders by `created` with no tiebreaker, so the template's column order is written into explicit per-field `created` stamps — the same trick `record.service.ts` uses for batches; and the delete passes `returnDeleted: false`, because materialising a `PopulatedRecord[]` nobody reads costs up to `MAX_RECORDS_PER_TABLE` × `MAX_FIELDS_PER_TABLE` rows while the transaction holds a pool connection. In `create` mode the template's `externalId` is preserved (that is what makes an export portable across projects) unless the project already has a table with it, in which case a fresh one is minted and the tool says so in its result — there is no unique index on `(projectId, externalId)`, and `getOneByExternalIdOrThrow` would resolve a duplicated pair arbitrarily. That check is check-then-create with no index to arbitrate, so two concurrent imports of the same template into one project can still both keep the externalId; closing that needs the unique index, i.e. a migration on a populated table. Note also that the per-field `created` stamps come from the API server's clock while every other field insert takes the database's `now()` default, so under clock skew a column added later through `POST /v1/fields` can sort before an imported one — display order only, since row import maps cells by `externalId` rather than position. The real fix is the `position` column already TODO'd in `table.service.ts`.
...
- `record.update()` — update cells. A cell **absent** from the request is unchanged; a cell sent with `''` or `null` is **cleared** (stored as `''`, which every type validates as "unset" and `not_exists` matches). The qadam's "leave a value empty to keep it" is a client-side rule, not this one — see Table → Flow Integration. Optional `precondition` (an array of `Filter`) makes it compare-and-set: evaluated under a `pessimistic_write` row lock inside the same transaction as the write, raising `RECORD_PRECONDITION_FAILED` (409) rather than silently writing nothing. The row lock is taken unconditionally, so a plain concurrent update cannot clobber between a conditional update's check and its write.
...
- **Write projection (#506)** — `update()`, `updateMany()` and `upsert()` take the same optional `fieldIds` as the reads. Each validates it with `resolveProjectedFields` against the table's fields **inside the write's transaction, before any cell is written** (both upsert branches, each where it loads the schema), so an unknown column is a 409 with nothing written. The narrowing itself is `recordService.projectForResponse`, applied by the **controller to the HTTP reply only**: the same record goes to the ON_NEW_RECORD / ON_UPDATE_RECORD webhooks, which keep receiving every column whatever the writing step asked to read back. In a JSON body `fieldIds: []` is reachable (unlike the query-string form) and is rejected by the schema rather than read as either "all" or "none".
...
- **Actions**: Create Record(s), Get Record, Find Records, Update Record, Update Record(s), Upsert Record(s), Delete Record(s), Clear Table
- **Empty means keep; Clear Columns means clear (#506).** On the three write actions a value left empty is filtered out before validation and never sent, so an unbound `{{...}}` never erases data. Emptying a cell is an explicit list: `clear_columns` on Update Record, a per-row `__clear` on Update Record(s) / Upsert Record(s) (namespaced like `__record_id`). Resolved by `columnUtils.toClearFieldIds` — an empty list clears nothing, an unknown column raises; a column both set and cleared raises; Upsert refuses to clear a key column, since an empty key component changes which record matches. The write actions also take `columns`, sent as the server-side `fieldIds` projection.
