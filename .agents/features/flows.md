...
## Summary
Flows are the core automation primitive in Qadam Flow. Each flow is a versioned directed graph of trigger and action steps stored as a JSONB tree. The module handles the full lifecycle: draft editing via a single-endpoint operation dispatch, publishing (locking a version and registering the trigger source), enabling/disabling, folder organization, sample data capture for testing, human-input forms/chat interfaces, and the visual builder frontend powered by XYFlow. All 26 flow modification types are dispatched through one endpoint (`POST /v1/flows/:id`) with a discriminated-union body.
...
- **Execution policy (#387, #374).** `settings.execution: { mode, maxConcurrency, rateLimit, onRateLimited, maxRateLimitRetries, onIterationFailure, tolerateFailures }`. One pool runs every loop (`SEQUENTIAL` is concurrency 1). Each iteration runs in a fork (`FlowExecutorContext.forkForIteration`): its own `currentPath` and verdict over the shared journal; `item`/`index` come from `loopItems` keyed by the loop's path, not the loop's one shared output. The pool merges `stepsCount`/`tags` and picks the run's verdict by priority (`LOG_SIZE_EXCEEDED` > `FAILED` > stop > `PAUSED`). A fork's verdict never reaches the progress reporter (reported as RUNNING), so a failed iteration under CONTINUE is not flushed as a FAILED run. `rateLimit` spaces starts evenly; a failure carrying `retryAfterSeconds` pauses the whole limiter and re-queues the item with its FAILED steps dropped (`isCompleted` treats FAILED as done). An iteration of a CONCURRENT loop cannot pause: `pauses: true` is refused before `run()`, and the `createWaitpoint`/`waitForWaitpoint` hooks throw before any waitpoint exists; `ap_validate_flow` reports it as `concurrent_pause`. The operator ceiling is `AP_LOOP_MAX_CONCURRENCY` (default 10), passed to the sandbox through worker settings. A loop inside a concurrent iteration — directly, or in an inline child called from one (`EngineConstants.insideConcurrentIteration`) — runs one item at a time, so nesting cannot multiply the ceiling. `onRateLimited: FAIL` always wins; otherwise a loop with `rateLimit` retries provider waits by default. Concurrent inline children of one job serialize their provisioning (a per-job `Mutex` in a `WeakMap` keyed on the job context).
- **Durable loop (#387).** `execution.durable` lets a top-level loop outlive one `FLOW_TIMEOUT_SECONDS` budget. Before starting each new item, and before waiting on the limiter, the pool checks: budget left under `min(max(60 s, 10%), 50%)` once an item finished in this execution (so a tiny budget cannot checkpoint forever), or the limiter wanting to wait over 60 s once an item was dispatched. The log's size is not a reason: a resume restores the whole journal, so a checkpoint would not shrink it — the log cap still fails the run. Provider retries already spent on a pending item ride along in `checkpoint.rateLimitedRetries`, so `maxRateLimitRetries` holds across checkpoints. A loop that cannot checkpoint fails as soon as its next start is spaced past the budget left, instead of sleeping in its slot. Then it stops dispatching, drains in-flight items, sets the loop step `PAUSED` with `output.checkpoint { count, lastAt, reason, itemsCount, itemsHash, rateLimitedRetries? }`, creates a DELAY waitpoint under the loop's own step name, and returns `PAUSED`. The RESUME is a new job with a fresh budget (`EngineConstants.executionStartedAt`); replay skips every `S` item. A resume against a different items list (count or hash) fails rather than shift positions. A durable loop defaults to `keepBodies: FAILED_ONLY` (`loopSettingsDefaults.keepBodies`, shared by the engine, the builder and the run view). It is a cooperative checkpoint, not crash recovery: a run killed mid-item (TIMEOUT, OOM) is retried by hand, and items in flight then are at-least-once. In an inline child a needed checkpoint fails the loop with a message (no job to resume); `ap_validate_flow` reports a callee's durable loop under Inline Subflows That Pause. A provider wait on a rate-limited item is honoured only when the loop opted into rate handling, up to 60 s in process or 24 h for a durable loop; past that the item fails with the wait on its error.
