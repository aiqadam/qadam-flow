...
`EngineConstants` holds the memoized state: `getTranslations()` (a `Map<key, Map<locale, value>>` — `Map` throughout, never a plain object, so a key or locale literally equal to `__proto__`/`constructor` is an ordinary entry, never a prototype lookup) and `getRunLocale({ currentState })` (evaluates `flowVersionLocaleSource` once, lazily, via the same `evalInScope` sandboxed-eval path the dynamic-locale bracket uses — a `localeSource` that fails to evaluate to a usable locale logs a warning and falls back, never fails the run).

### Subflow locale propagation

- **Inline `callFlow`** (`inline-flow-executor.ts`): the parent's resolved run locale is read via `parentConstants.getRunLocale({ currentState: {} })` and passed as a plain field into the child's `EngineConstants` — no wire format needed, same process. The empty scope is a known simplification: if the parent's `localeSource` has not been resolved yet by the time an inline `callFlow` step runs (no `$t` used earlier), it is evaluated against no step outputs, which only matters for a `localeSource` expression that itself reads step data.
- **Queued `callFlow`**: the wire format is wired end-to-end on the *consumer* side — `PARENT_RUN_LOCALE_HEADER` (shared), `webhook-request-converter.ts` → `webhook.service.ts` → `flow-run-service.ts` (`start`/`addToQueue`) → `ExecuteFlowJobData.inheritedRunLocale` / `WebhookJobData.inheritedRunLocale` → `execute-flow.ts` / `execute-webhook.ts` (worker) → `BeginExecuteFlowOperation.inheritedRunLocale` → `EngineConstants`. **The producer side is not implemented in this PR**: nothing currently sends `PARENT_RUN_LOCALE_HEADER` from a queue-mode `callFlow` step, because populating it needs a `context.run.locale()` hook on `packages/qadams/framework`'s `RunContext` plus a change in `packages/qadams/core/subflows`' `call-flow.ts` — two more published packages for a narrower behavior detail than the inline path. Until that follow-up ships, a queued subflow child always falls back to its own project's `defaultLocale`.

## `FlowVersion.localeSource`

A nullable, versioned string field (present in `FlowVersionTemplate`/`SharedTemplate` for free, since it derives from `FlowVersion` via `.omit()`). Evaluated the same way as the `$t[...][expr]` dynamic locale bracket — raw JS via `evalInScope` against the current step scope, not a mustache-wrapped template string (a design decision made in this PR: the plan specified the resolution *chain*, not how `localeSource` itself is evaluated; reusing the dynamic-locale bracket's own evaluation style keeps one mechanism for both expression surfaces this feature adds). Set via `FlowOperationType.UPDATE_LOCALE_SOURCE`; carried by `IMPORT_FLOW`, `ap_build_flow`'s optional `localeSource` input, and `ap_import_flow`. No `FlowVersion.schemaVersion` bump: unlike the v21 step-output-nesting migration, this is a new independent field with a natural `null` default — existing content needs no rewriting.
