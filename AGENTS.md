...
- **A `StaticDropdown`/`StaticMultiSelectDropdown` prop's own `defaultValue` must be one of its own declared `options`** — the builder seeds every form field from `defaultValue`, so a mismatch hands the user a default the property's own declaration calls invalid (#427). `npm run check-dropdown-defaults` statically scans every qadam for this and runs in CI; it only catches a literal `defaultValue` against a literal `options` array, so a dynamically-built dropdown (`.map()`, an imported constant) is on you to get right by hand. If the value is a deliberate "unset" sentinel (e.g. `''` on an optional filter with no natural default), drop `defaultValue` entirely rather than adding a fake option for it.
