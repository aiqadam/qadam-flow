...
// actually in — seven of fourteen skills were routed to from nowhere outside their own
// directory (the other seven from one AGENTS.md table row that named five of them and two
// path mentions in docs/README.md; `db-migration` alone was routed to from several places),
// two shipped with no YAML frontmatter at all (so their "description" was whatever the
// harness derived from the H1: "Browser Testing with agent-browser"), and three of five
// charters had no documented trigger. Nothing was red, because nothing was checked.
//
...
// skill", "the `server` agent", "the `app-sec` reviewer", or a path under `.agents/`. Only
// those shapes are treated as a reference — a bare backticked token is far more often a
// symbol name than a skill, and a gate with false positives on every AGENTS.md is a gate
// people disable. The `reviewer` spelling is here because the two mandatory charters are
// naturally written as reviewers, and the first version of this gate saw none of those four
// references as a result.
const REFERENCE_PATTERNS = [
  { pattern: /`([a-z0-9][a-z0-9-]*)`\s+skill\b/g, kind: 'skill' },
  { pattern: /`([a-z0-9][a-z0-9-]*)`(?:'s)?\s+(?:agent|reviewer)s?\b/g, kind: 'agent' },
...
const checkDocsLinked = ({ root }) => {
  ...
  const docs = fs.readdirSync(dir).filter((name) => name.endsWith('.md'))
  if (docs.length === 0) {
    return [`${DOCS_DIR}/ contains no deep dives (root: ${root}) — is --root correct, or did they move?`]
  }
...
const checkRoutingReferences = ({ root }) => {
  ...
  const files = findAgentsDocs({ root })
  if (files.length === 0) {
    return [`no AGENTS.md found anywhere under ${root} — the routing check scanned nothing.`]
  }
    ...
    content.split('\n').forEach((line, index) => {
      ...
      for (const { pattern, kind } of REFERENCE_PATTERNS) {
        for (const match of line.matchAll(pattern)) {
          if (!known[kind].includes(match[1])) {
            missing.add(`${kind} "${match[1]}"`)
          }
        }
      }
      for (const reference of missing) {
        problems.push(`${relative}:${index + 1}: routes to ${reference}, which does not exist — a routing table that outlives the file it points at is worse than none.`)
      }
    })
  }
  return problems
...
const findAgentsDocs = ({ root }) => {
  ...
  const walk = (dir) => {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        // A Dirent is lstat-shaped, so the .claude/.cursor mirrors report isDirectory() === false
        // and are never descended — no AGENTS.md is found twice through them.
