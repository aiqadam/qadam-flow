...
echo "== a symbol in backticks is not mistaken for a routing reference =="
new_root
printf '\nThe `projectId` column and `npm run lint-dev` are not skills.\n' >> "$root/packages/demo/AGENTS.md"
run_check
expect_status 0 "only \"<name>\` skill/agent\" and .agents/ paths count as references"
