...
validate_port() {
  case "$QADAM_FLOW_PORT" in
    ''|*[!0-9]*) die "QADAM_FLOW_PORT must be a number between 1 and 65535 (got '$QADAM_FLOW_PORT')" ;;
  esac
  if [ "$QADAM_FLOW_PORT" -lt 1 ] || [ "$QADAM_FLOW_PORT" -gt 65535 ]; then
    die "QADAM_FLOW_PORT must be a number between 1 and 65535 (got '$QADAM_FLOW_PORT')"
  fi
}
...
reconcile_port() {
  ...
  if [ "$frontend_url" = "http://localhost:${existing_port:-$DEFAULT_PORT}" ]; then
