...
for i in $(seq 1 300); do
  HAS_WEBHOOK=$(curl -sf "$BASE_URL/qadams" 2>/dev/null | jq '[.[].name] | any(. == "@aiqadam/qadam-webhook")' 2>/dev/null || echo "false")
...
SIGNUP_RESPONSE=$(curl -s --fail-with-body "$BASE_URL/authentication/sign-up" \
  ...
  -d '{"email":"bench@aiqadam.org","password":"BenchmarkPass1","firstName":"Bench","lastName":"Mark","trackEvents":false,"newsLetter":false}')
  ...
  '{
    ...
    request: {
      ...
      trigger: {
        ...
        type: "PIECE_TRIGGER",
        settings: {
          pieceName: "@aiqadam/qadam-webhook",
          pieceVersion: "~0.1.29",
