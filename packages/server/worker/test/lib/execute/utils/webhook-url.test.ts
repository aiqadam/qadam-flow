...
describe('regression: flow.aiqadam.org double-slash bug', () => {
    const flowId = 'flow-abc'

    it('old behavior would produce double slash — fixed behavior produces correct URL', () => {
        // Before the fix, PUBLIC_URL was used directly (e.g. "https://flow.aiqadam.org/")
        // which produced "https://flow.aiqadam.org//v1/webhooks/flow-abc"
        const publicUrl = 'https://flow.aiqadam.org/'
        const apiUrl = ensurePublicApiUrl(publicUrl)
        expect(apiUrl).toBe('https://flow.aiqadam.org/api/')
        expect(getWebhookUrl(apiUrl, flowId))
            .toBe('https://flow.aiqadam.org/api/v1/webhooks/flow-abc')
    })
    ...
    it('produces correct app webhook URL for cloud', () => {
        ...
        const apiUrl = ensurePublicApiUrl(publicUrl)
        expect(getAppWebhookUrl(apiUrl, 'slack'))
            .toBe('https://flow.aiqadam.org/api/v1/app-events/slack')
    })
})
