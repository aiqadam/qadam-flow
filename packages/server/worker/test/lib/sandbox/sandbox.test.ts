...
import { EngineResponseStatus, ErrorCode, QadamFlowError, WorkerContract } from '@aiqadam/shared'
...
function createTestProcessMaker() {
    ...
    const maker: SandboxProcessMaker = {
        create: vi.fn(async (params) => {
            ...
            Object.assign(child, { pid: 12345, exitCode: null, kill: vi.fn() })
...
describe('createSandbox', () => {
    ...
    describe('start', () => {
        ...
        it.each([
            ...
            expect((caughtErr as QadamFlowError).error.code).toBe(ErrorCode.VALIDATION)
    ...
    describe('execute', () => {
        ...
        it('rejects with SANDBOX_EXECUTION_TIMEOUT on real timeout', async () => {
            ...
            try {
                ...
                expect((err as QadamFlowError).error.code).toBe(ErrorCode.SANDBOX_EXECUTION_TIMEOUT)
            }
        })
        ...
        it('rejects with SANDBOX_MEMORY_ISSUE on exit code 134 / SIGABRT', async () => {
            ...
            try {
                ...
                expect((err as QadamFlowError).error.code).toBe(ErrorCode.SANDBOX_MEMORY_ISSUE)
            }
        })
        ...
        it('rejects with SANDBOX_LOG_SIZE_EXCEEDED', async () => {
            ...
            try {
                ...
                expect((err as QadamFlowError).error.code).toBe(ErrorCode.SANDBOX_LOG_SIZE_EXCEEDED)
            }
        })
        ...
        it('rejects with SANDBOX_INTERNAL_ERROR for other exit codes', async () => {
            ...
            try {
                ...
                expect((err as QadamFlowError).error.code).toBe(ErrorCode.SANDBOX_INTERNAL_ERROR)
            }
        })
