...
function getAppWebhookDetails(flowVersion: FlowVersion, publicApiUrl: string, appWebhookSecretsJson: string): { appWebhookUrl?: string, webhookSecret?: string | Record<string, string> } {
    const trigger = flowVersion.trigger as QadamTrigger
    const qadamName = trigger?.settings?.qadamName
    if (isNil(qadamName)) {
        return {}
    }
    ...
    const webhookSecret = secrets?.[qadamName]?.webhookSecret
    const pieceUrlName = qadamName.replace('@aiqadam/qadam-', '')
    return {
