            ...
            nextAction: {
                ...
                nextAction: {
                    ...
                    settings: {
                        ...
                        input: {
                            [AgentQadamProps.AI_PROVIDER_MODEL]: {
                                provider: 'GOOGLE',
...
describe('migrateV20GoogleModelPrefix', () => {
    ...
    it('strips the models/ prefix from a run_agent aiProviderModel.model', async () => {
        const result = await migrateV20GoogleModelPrefix.migrate(makeFlowVersion())
        const aiProviderModel = findStep(result, 'run_agent')[AgentQadamProps.AI_PROVIDER_MODEL] as Record<string, unknown>
        expect(aiProviderModel.model).toBe('gemini-2.5-pro')
    })

    it('leaves a non-prefixed managed model id untouched', async () => {
        const result = await migrateV20GoogleModelPrefix.migrate(makeFlowVersion())
        const aiProviderModel = findStep(result, 'run_agent_managed')[AgentQadamProps.AI_PROVIDER_MODEL] as Record<string, unknown>
