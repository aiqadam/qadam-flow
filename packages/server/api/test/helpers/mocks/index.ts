...
import {
    ...
    UserInvitation,
    UserStatus } from '@aiqadam/shared'
...
import { QadamTagSchema } from '../../../src/app/qadams/tags/qadams/qadam-tag.entity'
import { TagEntitySchema } from '../../../src/app/qadams/tags/tag-entity'
...
export const createMockPlan = (plan?: Partial<ProjectPlan>): ProjectPlan => {
    return {
        ...
        pieces: plan?.pieces ?? [],
        piecesFilterType: plan?.piecesFilterType ?? QadamsFilterType.NONE,
    }
}
...
export const createMockPlatform = (platform?: Partial<Platform>): Platform => {
    return {
        ...
        filteredQadamBehavior:
            ...
            faker.helpers.enumValue(FilteredQadamBehavior),
...
export const createMockQadamTag = (request: Partial<Omit<QadamTagSchema, 'platform' | 'tag'>>): Omit<QadamTagSchema, 'platform' | 'tag'> => {
    return {
        id: request.id ?? apId(),
        ...
        qadamName: request.qadamName ?? faker.lorem.word(),
        tagId: request.tagId ?? apId(),
    }
}

export const createMockQadamMetadata = (
    qadamMetadata?: Partial<Omit<QadamMetadataSchema, 'project'>>,
): Omit<QadamMetadataSchema, 'project'> => {
    return {
        id: qadamMetadata?.id ?? apId(),
        projectUsage: 0,
        created: qadamMetadata?.created ?? faker.date.recent().toISOString(),
        ...
        authors: qadamMetadata?.authors ?? [],
        platformId: qadamMetadata?.platformId,
        version: qadamMetadata?.version ?? faker.system.semver(),
        minimumSupportedRelease: qadamMetadata?.minimumSupportedRelease ?? '0.0.0',
        maximumSupportedRelease: qadamMetadata?.maximumSupportedRelease ?? '9.9.9',
        actions: qadamMetadata?.actions ?? {},
        triggers: qadamMetadata?.triggers ?? {},
        qadamType: qadamMetadata?.qadamType ?? faker.helpers.enumValue(QadamType),
        packageType:
            qadamMetadata?.packageType ?? faker.helpers.enumValue(PackageType),
...
export const createMockConnection = (connection: Partial<AppConnection>, ownerId: string): AppConnection<AppConnectionType.SECRET_TEXT> => {
    return {
        ...
        qadamName: connection?.qadamName ?? faker.lorem.word(),
        ...
        owner: null,
        qadamVersion: connection?.qadamVersion ?? '0.0.0',
...
export const mockAndSaveBasicSetup = async (params?: MockBasicSetupParams): Promise<MockBasicSetup> => {
    ...
    const mockPlatform = createMockPlatform({
        ...
        filteredQadamBehavior: params?.platform?.filteredQadamBehavior ?? FilteredQadamBehavior.BLOCKED,
    })
...
export const mockQadamMetadata = async (mockLog: FastifyBaseLogger): Promise<QadamMetadata> => {
    const { mockPlatform } = await mockAndSaveBasicSetup()
    const mockQadamMetadata = createMockQadamMetadata({
        platformId: mockPlatform.id,
        packageType: PackageType.REGISTRY,
    })
    await databaseConnection().getRepository('qadam_metadata').save([mockQadamMetadata])
    qadamMetadataService(mockLog).getOrThrow = vi.fn().mockResolvedValue(mockQadamMetadata)
    return mockQadamMetadata
}
