...
describe('POST /v1/flows/:id IMPORT_FLOW migration', () => {
    ...
    it('lets the schema answer when the migration fails on a body the schema rejects anyway', async () => {
        const ctx = await createTestContext(app!)
        const flow = createMockFlow({ projectId: ctx.project.id })
        const version = createMockFlowVersion({ flowId: flow.id, displayName: 'Before import' })
        await db.save('flow', flow)
        await db.save('flow_version', version)
        vi.spyOn(flowMigrationsModule, 'migrateFlowVersionTemplate').mockRejectedValue(new Error('cannot walk'))

        const response = await ctx.post(`/v1/flows/${flow.id}`, {
            type: FlowOperationType.IMPORT_FLOW,
            request: { displayName: 'After import', schemaVersion: '1', notes: null, trigger: {} },
        })
