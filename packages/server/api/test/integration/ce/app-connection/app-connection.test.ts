...
describe('AppConnection CE API', () => {
    describeWithAuth('POST /v1/app-connections (Create)', () => app!, (setup) => {
        it('should create a SECRET_TEXT connection', async () => {
            ...
            expect(body.qadamName).toBe(mockPiece.name)
    ...
    describeWithAuth('GET /v1/app-connections (List)', () => app!, (setup) => {
        ...
        it('should filter by qadamName', async () => {
            const ctx = await setup()
            ...
            qadamMetadataService(mockLog).getOrThrow = vi.fn().mockResolvedValue(mockPieceA)

            await ctx.post('/v1/app-connections', {
                externalId: 'filter-a',
                displayName: 'Connection A',
                qadamName: mockPieceA.name,
                projectId: ctx.project.id,
                type: AppConnectionType.SECRET_TEXT,
                value: { type: AppConnectionType.SECRET_TEXT, secret_text: 's' },
                qadamVersion: mockPieceA.version,
            })

            qadamMetadataService(mockLog).getOrThrow = vi.fn().mockResolvedValue(mockPieceB)

            await ctx.post('/v1/app-connections', {
                externalId: 'filter-b',
                displayName: 'Connection B',
                qadamName: mockPieceB.name,
                projectId: ctx.project.id,
                type: AppConnectionType.SECRET_TEXT,
                value: { type: AppConnectionType.SECRET_TEXT, secret_text: 's' },
                qadamVersion: mockPieceB.version,
            })
            ...
            expect(body.data[0].qadamName).toBe(mockPieceA.name)
        })
    })
