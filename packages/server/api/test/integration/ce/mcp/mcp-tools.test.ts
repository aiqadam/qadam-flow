...
beforeAll(async () => {
    ...
    const gmailPiece = createMockQadamMetadata({
        name: '@aiqadam/qadam-test-email',
    ...
    await db.save('qadam_metadata', gmailPiece)

    const arrayPiece = createMockQadamMetadata({
        name: '@aiqadam/qadam-test-array',
    ...
    await db.save('qadam_metadata', arrayPiece)

    const dynamicPiece = createMockQadamMetadata({
        name: '@aiqadam/qadam-test-dynamic',
...
describe('MCP Tools integration', () => {
    ...
    it('12. ap_get_piece_props — returns error for non-existent piece', async () => {
        ...
        const result = await apGetPiecePropsTool(mcp, mockLog).execute({
            qadamName: '@aiqadam/qadam-nonexistent',
    ...
    it('17. ap_validate_step_config — PIECE_ACTION missing qadamName returns error', async () => {
        const ctx = await createTestContext(app)
        const mcp = makeMcp(ctx.project.id)
        ...
        expect(text(result)).toContain('❌')
        expect(text(result)).toContain('qadamName')
    })
    ...
    it('34. ap_get_piece_props — DYNAMIC properties show note when refreshers not provided', async () => {
        ...
        const result = await apGetPiecePropsTool(mcp, mockLog).execute({
            qadamName: '@aiqadam/qadam-test-dynamic',
    ...
    it('67. ap_get_piece_props — ARRAY property includes item sub-schemas', async () => {
        ...
        const result = await apGetPiecePropsTool(mcp, mockLog).execute({
            qadamName: '@aiqadam/qadam-test-array',
    ...
    // on that platform (qadamType: CUSTOM) are discoverable. Without the
    // platformId filter, `qadamCache.filterQadamBasedOnType` drops them.

    it('90. ap_research_pieces — includes CUSTOM pieces belonging to the caller\'s platform', async () => {
        const ctx = await createTestContext(app)
        const mcp = makeMcp(ctx.project.id)

        const privatePiece = createMockQadamMetadata({
            name: '@aiqadam/qadam-private-custom',
        ...
        await db.save('qadam_metadata', privatePiece)
        ...
        expect(text(result)).toContain('✅')
        expect(text(result)).toContain('@aiqadam/qadam-private-custom')
    })
    ...
    it('91. ap_get_piece_props — resolves CUSTOM pieces on the caller\'s platform via lookupPieceComponent', async () => {
        ...
        const privatePiece = createMockQadamMetadata({
            name: '@aiqadam/qadam-private-lookup',
        ...
        const result = await apGetPiecePropsTool(mcp, mockLog).execute({
            qadamName: '@aiqadam/qadam-private-lookup',
    ...
    it('92. ap_research_pieces — does NOT include CUSTOM pieces from a different platform', async () => {
        ...
        const privateOfB = createMockQadamMetadata({
            name: '@aiqadam/qadam-private-only-b',
        ...
        await db.save('qadam_metadata', privateOfB)
        ...
        expect(text(result)).toContain('✅')
        expect(text(result)).not.toContain('@aiqadam/qadam-private-only-b')
    })
