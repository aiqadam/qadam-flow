...
export const appConnectionService = (log: FastifyBaseLogger) => ({
    async upsert(params: UpsertParams): Promise<AppConnectionWithoutSensitiveData> {
        const { projectIds, externalId, value, displayName, qadamName, ownerId, platformId, scope, type, status, metadata, preSelectForNewProjects } = params
        const qadamVersion = params.qadamVersion ?? ( await qadamMetadataService(log).getOrThrow({
            name: qadamName,
            platformId,
        })).version
        validatePieceVersion(qadamVersion)
        ...
        if (status === AppConnectionStatus.MISSING) {
            ...
            if (!isNil(existingForPlaceholder) && existingForPlaceholder.status !== AppConnectionStatus.MISSING) {
                log.info({ connectionId: existingForPlaceholder.id, qadamName, platformId, existingStatus: existingForPlaceholder.status }, 'Placeholder upsert skipped — non-missing connection already exists')
    ...
    async replace(params: ReplaceParams): Promise<void> {
        ...
        if (sourceAppConnection.qadamName !== targetAppConnection.qadamName) {
            throw new QadamFlowError({
    ...
    async list({
        ...
        if (!isNil(qadamName)) {
            querySelector.qadamName = Equal(qadamName)
        }
...
const validateConnectionValue = async (
    ...
    const { value, qadamName, qadamVersion, projectId, platformId } = params
...
const engineValidateAuth = async (
    ...
    const { qadamName, auth, projectId, platformId } = params
    ...
    const engineResponse = await userInteractionWatcher.submitAndWaitForResponse<EngineResponse<ExecuteValidateAuthResponse>>({
        qadam: await getQadamPackageWithoutArchive(log, platformId, {
            qadamName,
...
function validatePieceVersion(qadamVersion: string): void {
    if (!semver.valid(qadamVersion)) {
        throw new QadamFlowError({
            code: ErrorCode.VALIDATION,
            params: {
                message: 'Invalid qadam version',
            },
        })
    }
}
