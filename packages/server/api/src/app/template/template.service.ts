import { apId, CreateTemplateRequestBody, ErrorCode, FlowVersionTemplate, isNil, ListTemplatesRequestQuery, QadamFlowError, SeekPage, spreadIfDefined, Template, TemplateStatus, TemplateType, UpdateTemplateRequestBody } from '@aiqadam/shared'
...
export const templateService = (log: FastifyBaseLogger) => ({
    ...
    async create({ platformId, params }: CreateParams): Promise<Template> {
        ...
        const { flows, qadams } = preparedTemplate
        ...
        switch (type) {
            ...
            case TemplateType.CUSTOM: {
                throw new QadamFlowError({
                    ...
                    params: { message: 'Custom platform templates are not supported' },
                })
            }
        }
    },
    ...
    async update({ id, params }: UpdateParams): Promise<Template> {
        ...
        let qadams: string[] | undefined = undefined
        if (!isNil(params.flows) && params.flows.length > 0) {
            ...
            qadams = preparedTemplate.qadams
        }
    ...
    async list({ platformId, qadams, tags, search, type, category }: ListParams): Promise<SeekPage<Template>> {
        const commonFilters: Record<string, unknown> = {}

        if (qadams) {
            commonFilters.qadams = ArrayOverlap(qadams)
        }
        if (category) {
