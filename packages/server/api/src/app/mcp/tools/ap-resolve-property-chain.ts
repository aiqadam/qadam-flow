...
export const apResolvePropertyChainTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        ...
        execute: async (args) => {
            try {
                const { qadamName, actionOrTriggerName, type, propertyChain, auth, currentInput: providedInput } = resolvePropertyChainInput.parse(args)
                ...
                if (isNil(firstPropDef)) {
                    return {
                        content: [{ type: 'text', text: `❌ Property "${propertyChain[0].propertyName}" not found on ${normalized}/${actionOrTriggerName}. Use ap_get_qadam_props to see available properties.` }],
                    }
                }
                const qadamPackage = await getQadamPackageWithoutArchive(log, platformId, { qadamName: normalized, qadamVersion: qadam.version })
                ...
                for (const chainItem of propertyChain) {
                    ...
                    let result: EngineResponse<{ options: Array<{ label: string, value: unknown }> | QadamPropertyMap, disabled?: boolean }>
                    try {
                    ...
                    if (propDef.type === PropertyType.DYNAMIC && isObject(options) && !Array.isArray(options)) {
                        const dynamicFields = mcpUtils.buildPropSummaries(options as QadamPropertyMap)
                        resolvedProperties.push({
                            propertyName: chainItem.propertyName,
                            dynamicFields,
                            resolved: true,
...
const resolvePropertyChainInput = z.object({
    qadamName: z.string().describe('The piece name (e.g. "@aiqadam/qadam-google-sheets").'),
