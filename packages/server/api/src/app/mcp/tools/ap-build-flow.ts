...
export const apBuildFlowTool = ({ mcp, userId }: McpToolContext, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        ...
        inputSchema: {
            ...
            trigger: z.object({
                qadamName: z.string().describe('Trigger piece name (e.g. "@aiqadam/qadam-webhook")'),
        ...
        execute: async (args) => {
            ...
            try {
                ...
                const triggerVersionResult = await mcpUtils.resolveLatestQadamVersion({ qadamName: trigger.qadamName, projectId, platformId, log })
                ...
                const triggerPayload = QadamTrigger.parse({
                    name: 'trigger',
                    ...
                    settings: {
                        qadamName: triggerVersionResult.normalizedPieceName,
                        qadamVersion: triggerVersionResult.qadamVersion,
                ...
                for (const step of steps) {
                    ...
                    if (step.type === FlowActionType.PIECE) {
                        ...
                        const versionResult = await mcpUtils.resolveLatestQadamVersion({ qadamName: step.qadamName, projectId, platformId, log })
                        if (versionResult.error) {
                            skippedSteps.push(step.displayName)
                            continue
                        }
                        resolvedPieceVersion = versionResult.qadamVersion
    ...
    switch (step.type) {
        ...
        case FlowActionType.PIECE:
            return {
                ...
                valid: false,
                settings: {
                    qadamName: resolvedPieceName ?? step.qadamName ?? '',
                    qadamVersion: resolvedPieceVersion ?? '',
