                ...
                if (updatedStep && !updatedStep.valid) {
                    ...
                    const hint = (diagnosis || null)
                        ?? (step.type === FlowActionType.PIECE
                            ? 'Use ap_research_pieces to verify qadamName, qadamVersion, actionName and required inputs, then retry.'
...
async function diagnoseMissingInputs({ settings, platformId, log }: {
    ...
    try {
        ...
        log.warn({ err, qadamName, actionName }, 'diagnoseMissingInputs: failed to fetch qadam metadata')
        return null
    }
}
