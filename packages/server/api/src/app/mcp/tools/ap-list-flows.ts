import { FlowStatus, FlowTriggerType, isNil, McpToolDefinition, Permission, PopulatedFlow, ProjectScopedMcpServer } from '@aiqadam/shared'
                ...
                const structured = {
                    flows: flows.data.map((flow) => ({
                        ...
                        triggerType: flow.version.trigger.type === FlowTriggerType.PIECE
                            ? (flow.version.trigger.settings.qadamName ?? 'qadam (unconfigured)')
