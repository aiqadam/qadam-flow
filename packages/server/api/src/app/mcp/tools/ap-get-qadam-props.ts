...
import { getQadamPackageWithoutArchive } from '../../qadams/metadata/qadam-metadata-service'
...
export const apGetPiecePropsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        ...
        execute: async (args) => {
            try {
                const { qadamName, actionOrTriggerName, type, auth, flowId, input: providedInput } = getPiecePropsInput.parse(args)
                ...
                const lookup = await mcpUtils.lookupQadamComponent({
                    qadamName,
                ...
                const { qadam, component, qadamName: normalized } = lookup
                const label = type === 'action' ? 'Action' : 'Trigger'
                const props = mcpUtils.buildPropSummaries(component.props)
                const requiresAuth = component.requireAuth && !isNil(qadam.auth)

                const hasRealProject = mcpUtils.isProjectScoped(mcp)
                let authHint: AuthHint | undefined
                if (hasRealProject && requiresAuth && auth) {
                    const authOwnership = await validateAuthOwnership({ auth, qadamName: normalized, projectId: mcp.projectId, platformId, log })
                    if (authOwnership) {
                ...
                if (requiresAuth && !auth) {
                    if (hasRealProject) {
                        authHint = await discoverAvailableConnections({ qadamName: normalized, projectId: mcp.projectId, platformId, log })
                    }
                    else {
...
async function resolvePropertyOptions({ props, componentProps, qadamName, qadamVersion, actionOrTriggerName, auth, flowId, providedInput, projectId, platformId, log }: ResolvePropertyOptionsParams): Promise<void> {
    ...
    const [qadamPackage, sampleData] = await Promise.all([
        getQadamPackageWithoutArchive(log, platformId, { qadamName, qadamVersion }),
        flow
        ...
        try {
            const result = await withTimeout({
                promise: userInteractionWatcher.submitAndWaitForResponse<EngineResponse<{
                    options: Array<{ label: string, value: unknown }> | QadamPropertyMap
                    disabled?: boolean
                }>>({
...
async function discoverAvailableConnections({ qadamName, projectId, platformId, log }: {
    qadamName: string
    projectId: string
    platformId: string
    ...
    try {
        ...
        log.debug({ err, qadamName }, 'Failed to discover connections')
        return { message: 'Use ap_list_connections to find connections.', connections: [] }
    }
}

async function validateAuthOwnership({ auth, qadamName, projectId, platformId, log }: {
    auth: string
    qadamName: string
    projectId: string
    platformId: string
    ...
    try {
        ...
        if (!match) {
            return {
                content: [{
                    type: 'text',
                    text: `⚠️ Connection "${auth}" does not belong to piece "${qadamName}". Use ap_list_connections to find the correct connection for this piece.`,
                }],
            }
        }
    }
...
const getPiecePropsInput = z.object({
    qadamName: z.string().describe('The piece name (e.g. "@aiqadam/qadam-slack"). Use ap_research_pieces to get valid values.'),
    actionOrTriggerName: z.string().describe('The action or trigger name (e.g. "send_channel_message"). Use ap_research_pieces with qadamNames to get valid values.'),
