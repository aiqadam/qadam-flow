...
const runActionInput = z.object({
    qadamName: z.string().describe('Piece name, e.g. "slack" or "@aiqadam/qadam-slack". Use ap_research_pieces to discover.'),
...
export const apRunActionTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        ...
        execute: async (args) => {
            try {
                const { qadamName, actionName, input, connectionExternalId } = runActionInput.parse(args)
