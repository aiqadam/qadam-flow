...
export async function executeAdhocAction({
    ...
    const normalizedPieceName = mcpUtils.normalizeQadamName(qadamName)
    if (isNil(normalizedPieceName)) {
        return mcpUtils.mcpToolError('Validation failed', new Error('qadamName is required'))
    }

    const lookup = await mcpUtils.lookupQadamComponent({
        qadamName: normalizedPieceName,
    ...
    const { qadam, component: action, qadamName: resolvedPieceName } = lookup
    ...
    const resolvedPieceVersion = qadamVersion ?? qadam.version
