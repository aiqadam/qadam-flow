            ...
            try {
                ...
                switch (params.stepType) {
                    ...
                    case 'PIECE_TRIGGER': {
                        ...
                        if (!params.qadamName || !componentName) {
                            const missing = !params.qadamName ? 'qadamName' : (componentType === 'action' ? 'actionName' : 'triggerName')
                            return { content: [{ type: 'text', text: `❌ ${missing} is required for ${componentType} validation.` }] }
                        }
                        return await validatePieceComponent({ qadamName: params.qadamName, componentName, componentType, input: params.input ?? {}, auth: params.auth, projectId: mcp.projectId, log })
                    }
                    case 'CODE':
...
const validateStepConfigInput = z.object({
    ...
    qadamName: z.string().optional()
        .describe('For PIECE_ACTION/PIECE_TRIGGER: piece name (e.g. "slack" or "@aiqadam/qadam-slack").'),
...
async function validatePieceComponent({ qadamName, componentName, componentType, input, auth, projectId, log }: ValidatePieceParams): Promise<McpToolResult> {
    const lookup = await mcpUtils.lookupQadamComponent({ qadamName, componentName, componentType, projectId, log })
    if (lookup.error) {
        return lookup.error
    }

    const { component, qadamName: normalized } = lookup
    const inputWithAuth = auth ? { ...input, auth } : input
    const diagnosis = mcpUtils.diagnoseQadamProps({
        props: component.props,
        input: inputWithAuth,
        qadamAuth: lookup.qadam.auth,
    ...
    if (diagnosis.missing.length === 0) {
        const uiHint = diagnosis.uiRequired.length > 0
            ? `\nNote: these fields require configuration in the Qadam Flow UI: ${diagnosis.uiRequired.join(', ')}.`
            : ''
        return {
