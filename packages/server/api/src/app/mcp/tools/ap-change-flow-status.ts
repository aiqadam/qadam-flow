        ...
        execute: async (args) => {
            ...
            if (status === FlowStatus.ENABLED && isNil(flow.publishedVersionId)) {
                return {
                    content: [{
                        type: 'text',
                        text: `❌ Flow ${mcpUtils.wrapUntrustedValue(flow.version.displayName)} has no published version. Use ap_lock_and_publish first.`,
                    }],
                }
            }
            ...
            if (status === FlowStatus.DISABLED && flow.status === FlowStatus.DISABLED) {
                return {
                    content: [{
                        type: 'text',
                        text: `✅ Flow ${mcpUtils.wrapUntrustedValue(flow.version.displayName)} is already disabled.`,
                    }],
                }
            }
            ...
            try {
                ...
                return {
                    content: [{ type: 'text', text: `✅ Flow ${mcpUtils.wrapUntrustedValue(flow.version.displayName)} ${action} successfully.` }],
                }
            }
            catch (err) {
