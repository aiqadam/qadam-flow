...
const researchPiecesSchema = z.object({
    qadamNames: z.array(z.string()).optional().describe('Exact piece names to look up (e.g. ["gmail", "slack", "@aiqadam/qadam-google-sheets"]). Always returns actions and triggers for each piece.'),
    categories: z.array(z.enum(Object.values(QadamCategory) as [string, ...string[]])).optional(),
...
export const apResearchPiecesTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        ...
        description: 'Research available pieces. Use qadamNames for bulk exact lookup (always returns actions and triggers). Use searchQuery for fuzzy discovery.',
        inputSchema: {
            qadamNames: researchPiecesSchema.shape.qadamNames,
        ...
        execute: async (args) => {
            try {
                ...
                if (params.qadamNames && params.qadamNames.length > 0) {
                    return await bulkLookup({
                        qadamNames: params.qadamNames,
                        projectId,
                        platformId,
                        log,
                    })
...
async function bulkLookup({ qadamNames, projectId, platformId, log }: {
    qadamNames: string[]
    projectId: string | undefined
    platformId: string
    log: FastifyBaseLogger
}): Promise<{ content: [{ type: 'text', text: string }], structuredContent: Record<string, unknown> }> {
    const capped = qadamNames.slice(0, BULK_LOOKUP_CAP)
    const svc = qadamMetadataService(log)
    const results = await Promise.all(capped.map(async (rawName) => {
        const normalized = mcpUtils.normalizeQadamName(rawName)
        if (isNil(normalized)) {
            return { name: rawName, found: false as const }
        }
        const qadam = await svc.get({ name: normalized, projectId, platformId })
        if (isNil(qadam)) {
        ...
        return {
            name: qadam.name,
            found: true as const,
            displayName: qadam.displayName,
            description: qadam.description,
            actions: Object.values(qadam.actions).map(summarizeComponent),
            triggers: Object.values(qadam.triggers).map(summarizeComponent),
        }
    }))
    ...
    if (qadamNames.length > BULK_LOOKUP_CAP) {
        hints.push(`⚠️ Only the first ${BULK_LOOKUP_CAP} pieces were looked up (${qadamNames.length} requested)`)
    }
...
async function searchPieces({ params, projectId, platformId, log }: {
    ...
    const pieces = await svc.list({
        ...
        platformId,
        includeHidden: false,
        categories: params.categories as QadamCategory[] | undefined,
        tags: params.tags,
