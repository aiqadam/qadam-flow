...
export const apResolvePropertyOptionsTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        ...
        execute: async (args) => {
            try {
                const { qadamName, actionOrTriggerName, type, propertyName, auth, input: providedInput, searchValue } = resolvePropertyOptionsInput.parse(args)
                ...
                if (isNil(propDef)) {
                    return {
                        content: [{ type: 'text', text: `❌ Property "${propertyName}" not found on ${normalized}/${actionOrTriggerName}. Use ap_get_qadam_props to see available properties.` }],
                    }
                }
...
const resolvePropertyOptionsInput = z.object({
    qadamName: z.string().describe('The piece name (e.g. "@aiqadam/qadam-slack").'),
