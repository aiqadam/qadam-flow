...
import { AIProviderName, isNil, McpToolDefinition, ProjectScopedMcpServer } from '@aiqadam/shared'
...
const setupGuideInput = z.object({
    ...
    qadamName: z.string().optional().describe('For connections: the piece that needs auth (e.g., "@aiqadam/qadam-gmail"). Omit for general instructions.'),
})
...
export const apSetupGuideTool = (mcp: ProjectScopedMcpServer, log: FastifyBaseLogger): McpToolDefinition => {
    return {
        ...
        execute: async (args) => {
            try {
                const { topic, qadamName } = setupGuideInput.parse(args)

                if (topic === 'connection') {
                    return await connectionGuide(mcp, log, qadamName)
                }
...
async function connectionGuide(mcp: ProjectScopedMcpServer, log: FastifyBaseLogger, qadamName?: string): Promise<{ content: [{ type: 'text', text: string }] }> {
    if (isNil(qadamName)) {
        return {
            content: [{
                type: 'text',
                text: [
                    ...
                    '',
                    'Tip: Pass qadamName to this tool for specific instructions (e.g., qadamName: "@aiqadam/qadam-gmail").',
                ].join('\n'),
            }],
        }
    }
