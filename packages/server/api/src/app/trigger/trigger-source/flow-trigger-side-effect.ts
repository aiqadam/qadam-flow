...
export const flowTriggerSideEffect = (log: FastifyBaseLogger) => {
    return {
        async enable(params: EnableFlowTriggerParams): Promise<ActiveTriggerReturn> {
            ...
            const { flowId, flowVersionId, projectId, simulate, qadamTrigger } = params
        ...
        async disable(params: DisableFlowTriggerParams): Promise<void> {
            ...
            switch (qadamTrigger.type) {
                case TriggerStrategy.APP_WEBHOOK:
                    ...
                    break
                case TriggerStrategy.WEBHOOK: {
                    const renewConfiguration = qadamTrigger.renewConfiguration
...
async function handleAppWebhookTrigger({ engineHelperResponse, flowId, projectId, qadamName }: ActiveTriggerParams): Promise<ActiveTriggerReturn> {
...
async function handleWebhookTrigger({ flowId, flowVersionId, projectId, qadamTrigger, log }: ActiveTriggerParams): Promise<ActiveTriggerReturn> {
    const renewConfiguration = qadamTrigger.renewConfiguration
