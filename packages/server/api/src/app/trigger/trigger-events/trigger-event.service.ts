...
function getSourceName(trigger: FlowTrigger): string {
    switch (trigger.type) {
        case FlowTriggerType.PIECE: {
            const qadamTrigger = trigger as QadamTrigger
            const qadamName = qadamTrigger.settings.qadamName
            const qadamVersion = getQadamMajorAndMinorVersion(
                qadamTrigger.settings.qadamVersion,
            )
            const triggerName = qadamTrigger.settings.triggerName
            return `${qadamName}@${qadamVersion}:${triggerName}`
        }
