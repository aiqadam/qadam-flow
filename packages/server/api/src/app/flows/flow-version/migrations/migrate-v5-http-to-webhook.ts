import { FlowActionType, flowStructureUtil, FlowVersion, PropertyExecutionType } from '@aiqadam/shared'
import semver from 'semver'
import { Migration } from '.'

const HTTP_PIECE_NAME = '@aiqadam/qadam-http'
    ...
    migrate: async (flowVersion: FlowVersion): Promise<FlowVersion> => {
        const newVersion = flowStructureUtil.transferFlow(flowVersion, (step) => {
            if (
                ...
                step.settings.qadamName === HTTP_PIECE_NAME &&
                step.settings.actionName === HTTP_RETURN_RESPONSE_ACTION
            ) {
                const httpInput = step.settings.input || {}
                const fields: Record<string, unknown> = {}
                const qadamVersionWithoutTildaOrPlus = step.settings.qadamVersion.replace('~', '').replace('^', '')
                // Check the scehma for each action in the http piece
                const isGreaterThanOrEqual050 = semver.gte(qadamVersionWithoutTildaOrPlus, '0.5.0')
