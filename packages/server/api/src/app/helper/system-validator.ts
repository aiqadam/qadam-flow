...
const systemPropValidators: {
    ...
    [AppSystemProp.LOOP_MAX_CONCURRENCY]: numberValidator,
    ...
    [AppSystemProp.LOAD_TRANSLATIONS_FOR_DEV_QADAMS]: booleanValidator,
...
export const validateEnvPropsOnStartup = async (log: FastifyBaseLogger): Promise<void> => {
    ...
    if (environment !== ApEnvironment.TESTING && fileStorageLocation === FileLocation.S3) {
        try {
            ...
            throw new Error(JSON.stringify({
                ...
                docUrl: 'https://flow.aiqadam.org/docs/install/configuration/overview#configure-s3-optional',
            }))
        }
    }
    ...
    if (!isNil(codeSandboxType)) {
        throw new Error(JSON.stringify({
            ...
            docUrl: 'https://flow.aiqadam.org/docs/install/configuration/overview',
        }))
    }
    ...
    if (!isValidHexKey) {
        throw new Error(JSON.stringify({
            ...
            docUrl: 'https://flow.aiqadam.org/docs/install/configuration/environment-variables',
        }))
    }
