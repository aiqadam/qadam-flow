import { assertNotNullOrUndefined, DeleteTagRequest, ListTagsRequest, PrincipalType, SeekPage, SetQadamTagsRequest, Tag, UpsertTagRequest } from '@aiqadam/shared'
import { FastifyPluginAsyncZod } from 'fastify-type-provider-zod'
import { StatusCodes } from 'http-status-codes'
import { z } from 'zod'
import { securityAccess } from '../../core/security/authorization/fastify-security'
import { qadamTagService } from './qadams/qadam-tag.service'
...
const tagsController: FastifyPluginAsyncZod = async (fastify) => {
    ...
    fastify.post('/qadams', setPiecesTagsParams, async (req, reply) => {
        const platformId = req.principal.platform.id
        const pieces = req.body.qadamsName.map(qadamName => qadamTagService.set(platformId, qadamName, req.body.tags))
