...
export function createHandlers(log: FastifyBaseLogger, workerGroupId?: string): WorkerToApiContract {
    return {
        ...
        async submitPayloads(input) {
            const { flowVersionId, projectId, payloads, httpRequestId, streamStepProgress, environment, parentRunId, failParentOnFailure, parentWaitpointId, parentSlotId, inheritedRunLocale } = input
        ...
        async getQadam(input) {
            return qadamMetadataService(log).get({
                name: input.name,
        ...
        async getUsedQadams() {
            ...
            const pieces = await distributedStore.get<QadamPackage[]>(redisKey)
            return pieces ?? []
        },

        async markQadamAsUsed(input) {
            const redisKey = `usedPieces:${workerGroupId ?? 'shared'}`
            const existing = await distributedStore.get<QadamPackage[]>(redisKey) ?? []
            const existingKeys = new Set(existing.map((p) => `${p.qadamName}@${p.qadamVersion}`))
            const newPieces = input.pieces.filter((p) => !existingKeys.has(`${p.qadamName}@${p.qadamVersion}`))
