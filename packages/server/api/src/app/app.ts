...
export const setupApp = async (app: FastifyInstance): Promise<FastifyInstance> => {
    ...
    await app.register(swagger as any, {
        ...
        openapi: {
            ...
            servers: [
                {
                    url: 'https://flow.aiqadam.org/api',
            ...
            info: {
                title: 'Qadam Flow API',
                version: '0.0.0',
            },
            externalDocs: {
                url: 'https://flow.aiqadam.org/docs',
    ...
    systemJobHandlers.registerJobHandler(SystemJobName.APPLY_DELIVERY_MODE_CHANGE, (data) => longPollingTransportChange(app.log).reEnableAffectedFlows({
        qadamName: data.qadamName,
...
The application started on ${await domainHelper.getPublicApiUrl({ path: '' })}, as specified by the AP_FRONTEND_URL variables.`)
    ...
    const devQadams = process.env.AP_DEV_QADAMS
    ...
    if (environment === ApEnvironment.DEVELOPMENT) {
        ...
        if (devQadams) {
            app.log.warn(
                `[WARNING]: Only qadams listed in AP_DEV_QADAMS will be loaded: ${devQadams}`,
            )
        }
    }
...
function registerOpenApiSchemas() {
    ...
    globalRegistry.add(QadamMetadata, { id: 'qadam' })
