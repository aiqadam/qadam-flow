import { EngineResponse, EngineResponseStatus, ExecuteTriggerResponse, FlowId, FlowVersionId, isNil, ProjectId, TriggerHookType, TriggerPayload, TriggerSource, WebhookHandshakeConfiguration, WebhookHandshakeStrategy, WorkerJobType } from '@aiqadam/shared'
...
export const webhookHandshake = {
    ...
    async getWebhookHandshakeConfiguration(params: GetWebhookHandshakeConfigurationParams): Promise<WebhookHandshakeConfiguration | null> {
        ...
        if (isNil(triggerSource) || isNil(triggerSource.qadamName) || isNil(triggerSource.qadamVersion) || isNil(triggerSource.triggerName) || isNil(triggerSource.projectId)) {
            return null
        }
        const qadamTrigger = await triggerUtils(logger).getQadamTriggerByName({
            qadamName: triggerSource.qadamName,
            qadamVersion: triggerSource.qadamVersion,
        ...
        return qadamTrigger?.handshakeConfiguration ?? null
    },
}
