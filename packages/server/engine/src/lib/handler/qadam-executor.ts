...
const executeAction: ActionHandler<QadamAction> = async ({ action, executionState, constants }) => {
    ...
    const { data: executionStateResult, error: executionStateError } = await utils.tryCatchAndThrowOnEngineError((async () => {
        ...
        const { qadamAction, qadam } = await qadamLoader.getQadamAndActionOrThrow({
            qadamName: action.settings.qadamName,
            qadamVersion: action.settings.qadamVersion,
            actionName: action.settings.actionName,
            devQadams: constants.devQadams,
        })

        const { resolvedInput, censoredInput } = await constants.getPropsResolver(qadam.getContextInfo?.().version).resolve<StaticPropsValue<QadamPropertyMap>>({
        ...
        const runMethodToExecute = (testSingleStepMode && !isNil(qadamAction.test)) ? qadamAction.test : qadamAction.run
        ...
        const isSamePiece = constants.triggerQadamName === action.settings.qadamName
