...
export const executionJournal = {
    ...
    upsertStep({ stepName, stepOutput, path, steps, createLoopIterationIfNotExists }: UpsertStepParams): Record<string, StepOutput> {
        ...
        setOwnStep(target, stepName, stepOutput)
        return steps
    },

    getStep({ stepName, path, steps }: GetStepParams): StepOutput | undefined {
        return getOwnStep(this.getStateAtPath({ path, steps }), stepName)
    },
    ...
    getStateAtPath({ path, steps }: GetStateAtPathParams): Record<string, StepOutput> {
        ...
        for (const [parentStepName, iteration] of path) {
            const step = getOwnStep(target, parentStepName)
            if (!step) {
    ...
    getOrCreateStateAtPath({ path, steps }: GetStateAtPathParams): Record<string, StepOutput> {
        ...
        for (const [parentStepName, iteration] of path) {
            let step = getOwnStep(target, parentStepName)
            if (!step ) {
            ...
            setOwnStep(target, parentStepName, loopStepOutput)
            target = iterationOutput
        }
        return target
    },
        ...
        Object.entries(steps).forEach(([stepName, step]) => {
            if (step.type === FlowActionType.LOOP_ON_ITEMS) {
                ...
                if (isNil(iterationsResult)) {
                    setOwnStep(result, stepName, step as LoopStepOutput)
                    return
                }
                result = {
                    ...result,
...
// has never run resolves those names off `Object.prototype` (a function, or — for `__proto__` — the
// object's own current prototype) instead of `undefined`, and every caller's `if (!step)` /
// `isNil(...)` guard treats that as "found".
...
// not create an own property at all — it invokes the inherited `Object.prototype.__proto__` setter
// and silently reassigns the target's own prototype instead. The step's own output then becomes
// invisible to `Object.keys`/`Object.entries`/`JSON.stringify` (i.e. the persisted run log) even
// though a direct read of that literal key still resolves it, which is how the step's result can
