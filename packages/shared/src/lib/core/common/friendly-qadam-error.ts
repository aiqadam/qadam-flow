...
const isFriendlyQadamError = (value: unknown): value is FriendlyQadamError => {
    if (!isObjectRecord(value)) {
        return false
    }
...
export const formatQadamError = (error: unknown, options?: FormatQadamErrorOptions): FriendlyQadamError => {
    const raw = options?.raw
    const withRaw = (base: FriendlyQadamError): FriendlyQadamError => {
...
export const tryParseFriendlyQadamError = (value: unknown): FriendlyQadamError | null => {
    if (isNil(value)) {
        return null
    }
    const candidate = isString(value) ? safeJsonParse(value) : value
    return isFriendlyQadamError(candidate) ? candidate : null
}
