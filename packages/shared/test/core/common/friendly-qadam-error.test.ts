...
describe('formatQadamError', () => {
    it('extracts status, request, response, and apiMessage from a Jira-style 403 HttpError', () => {
        ...
        const result = formatQadamError(error)
...
<Error><Code>NoSuchKey</Code><Message>The specified key does not exist.</Message></Error>`
    ...
    it('returns a default message for nil input', () => {
        expect(formatQadamError(null).message).toBe('Unknown error')
        expect(formatQadamError(undefined).message).toBe('Unknown error')
    })
    ...
    it('treats a string input as the message, stripping any stack frames', () => {
        ...
        const result = formatQadamError(raw)

        expect(result.message).toBe('HttpError: something went wrong')
        expect(result.message).not.toContain('at ')
    })

    it('returns the input verbatim if it is already a FriendlyQadamError', () => {
        const original = formatQadamError(new TestHttpError({ status: 404, body: { message: 'not found' } }, { body: {} }))
        const second = formatQadamError(original)
        expect(second).toBe(original)
    })

    it('fills in raw on an already-formatted error that has none', () => {
        const original = formatQadamError(new Error('Boom'))
        expect(original.raw).toBeUndefined()

        const second = formatQadamError(original, { raw: 'Error: Boom\n    at x' })
    ...
    it('does not overwrite an existing raw on an already-formatted error', () => {
        const original = formatQadamError(new Error('Boom'), { raw: 'original raw' })

        const second = formatQadamError(original, { raw: 'new raw' })
    ...
    it('attaches the raw dump when provided in options', () => {
        ...
        const result = formatQadamError(error, { raw: 'Error: Boom\n    at somewhere' })
    ...
    it('does not set raw when no raw option is provided', () => {
        const result = formatQadamError(new Error('Boom'))

        expect(result.raw).toBeUndefined()
    })

    it('truncates a very long raw dump', () => {
        const result = formatQadamError(new Error('Boom'), { raw: 'y'.repeat(20000) })
...
describe('tryParseFriendlyQadamError', () => {
    it('parses a JSON-stringified FriendlyQadamError', () => {
        const original = formatQadamError(new TestHttpError({ status: 429, body: { message: 'Too many requests' } }, { body: {} }))
        const json = JSON.stringify(original)
        const parsed = tryParseFriendlyQadamError(json)
        expect(parsed).not.toBeNull()
        expect(parsed?.status).toBe(429)
        expect(parsed?.apiMessage).toBe('Too many requests')
    })

    it('returns the value if already a FriendlyQadamError object', () => {
        const original = formatQadamError(new Error('x'))
        expect(tryParseFriendlyQadamError(original)).toEqual(original)
    })

    it('returns null for non-structured strings', () => {
        expect(tryParseFriendlyQadamError('not a friendly error')).toBeNull()
        expect(tryParseFriendlyQadamError('{"foo":"bar"}')).toBeNull()
    })

    it('returns null for nil values', () => {
        expect(tryParseFriendlyQadamError(null)).toBeNull()
