...
const { createToolName, createQadamToolName } = mcpToolNameUtils
...
describe('createQadamToolName', () => {
    it('strips @scope/qadam- prefix', () => {
        expect(createQadamToolName('@aiqadam/qadam-slack', 'send_message')).toBe('slack-send_message_pqyv3q_mcp')
    })

    it('strips plain qadam- prefix', () => {
        expect(createQadamToolName('qadam-github', 'create_issue')).toBe('github-create_issue_gmsjqn_mcp')
    })

    it('handles names without qadam- prefix', () => {
        expect(createQadamToolName('slack', 'send_message')).toBe('slack-send_message_pqyv3q_mcp')
    })

    it('normalizes the combined name correctly', () => {
        expect(createQadamToolName('@aiqadam/qadam-google-sheets', 'insert_row')).toBe('google-sheets-insert_row_q388b6_mcp')
    })
})
