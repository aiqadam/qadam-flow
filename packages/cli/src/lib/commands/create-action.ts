...
import { assertQadamExists, displayNameToCamelCase, displayNameToKebabCase, findQadam } from '../utils/qadam-utils';
...
function createActionTemplate(displayName: string, description: string) {
  ...
  const actionTemplate = `import { createAction, Property } from '@aiqadam/qadams-framework';
...
const createAction = async (qadamName: string, displayActionName: string, actionDescription: string) => {
  ...
  const actionsFolder = join(qadamFolder!, 'src', 'lib', 'actions')
...
export const createActionCommand = new Command('create')
  ...
  .action(async () => {
    ...
    createAction(answers.qadamName, answers.actionName, answers.actionDescription);
  });
