import { FriendlyQadamError, isNil } from '@aiqadam/shared';
...
const buildContextBlock = ({
  ...
  const lines: string[] = [];
  if (context.qadamDisplayName || context.qadamName) {
    const version = context.qadamVersion ? ` v${context.qadamVersion}` : '';
    lines.push(
      `Piece: ${context.qadamDisplayName ?? context.qadamName}${version}`,
    );
  }
  if (context.qadamAuthType) {
    lines.push(`Piece auth type: ${context.qadamAuthType}`);
  }
