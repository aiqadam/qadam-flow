...
import { AutoFormFieldWrapper } from '@/app/builder/qadam-properties/auto-form-field-wrapper';
...
function ConnectionSelect(params: ConnectionSelectProps) {
  ...
  const { qadamModel: pieceWithCorrectVersion, isLoading: isLoadingPiece } =
    qadamsHooks.useQadam({
      name: params.piece.name,
      version: reconnectConnection?.qadamVersion ?? params.piece.version,
    });
  const form = useFormContext<QadamAction | QadamTrigger>();
  ...
  const {
    ...
    request: {
      qadamName: params.piece.name,
      projectId: authenticationSession.getProjectId()!,
      limit: 1000,
    },
    qadamAuth: params.piece.auth,
