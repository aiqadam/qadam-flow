...
const PieceSettings = React.memo((props: PieceSettingsProps) => {
  ...
  const actionName = (props.step.settings as QadamActionSettings).actionName;
  const selectedAction = actionName
    ? qadamModel?.actions[actionName]
    : undefined;
  const triggerName = (props.step.settings as QadamTriggerSettings).triggerName;
  const selectedTrigger = triggerName
    ? qadamModel?.triggers[triggerName]
    : undefined;
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      {qadamModel && (
        <>
          {qadamModel.auth && (showAuthForAction || showAuthForTrigger) && (
            <ConnectionSelect
          ...
          {selectedAction && (
            <GenericPropertiesForm
              ...
              dynamicPropsInfo={{
                qadamName: qadamModel.name,
                qadamVersion: qadamModel.version,
