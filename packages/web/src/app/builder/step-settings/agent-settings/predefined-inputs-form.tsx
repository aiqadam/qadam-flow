import { QadamProperty, PropertyType } from '@aiqadam/qadams-framework';
import { FieldControlMode, isNil, PredefinedInputField } from '@aiqadam/shared';
...
import { selectGenericFormComponentForProperty } from '../../qadam-properties/properties-utils';
...
export const PredefinedInputsForm = () => {
  ...
  const selectedQadam = qadams?.find((p) => p.name === piece?.qadamName);
  ...
  useEffect(() => {
    const subscription = form.watch((values, { name }) => {
      ...
      const currentPredefined =
        useQadamToolsDialogStore.getState().predefinedInputs;
  ...
  const pieceHasAuth = requireAuth && selectedQadam?.auth;
  return (
    <Form {...form}>
      <ScrollArea className="h-full">
        <div className="flex items-start border-b gap-3 p-4">
          <div className="flex size-11 shrink-0 items-center justify-center rounded-sm border bg-background">
            <img
              ...
              src={selectedQadam?.logoUrl}
              alt={selectedQadam?.displayName}
            />
          </div>
        ...
        <div className="space-y-6 p-4">
          {pieceHasAuth && !isNil(selectedQadam) && (
            <ConnectionDropdown
              piece={selectedQadam}
                      ...
                      <FormField
                        ...
                        render={({ field }) =>
                          selectGenericFormComponentForProperty({
                            ...
                            form,
                            dynamicPropsInfo: {
                              qadamName: selectedQadam?.name ?? '',
                              qadamVersion: selectedQadam?.version ?? '',
