...
const DataSelector = ({ parentHeight, parentWidth }: DataSelectorProps) => {
  ...
  const pieceQueries = useQueries({
    queries: piecePairs.map(({ qadamName, qadamVersion }) => ({
      queryKey: ['piece', qadamName, qadamVersion],
      queryFn: () =>
        qadamsApi.get({
          name: qadamName,
  ...
  const schemaMap = useMemo<Record<string, OutputSchema | null>>(() => {
    ...
    piecePairs.forEach(({ stepName, stepKey }, idx) => {
      const piece = pieceQueries[idx]?.data as QadamMetadataModel | undefined;
      result[stepName] =
