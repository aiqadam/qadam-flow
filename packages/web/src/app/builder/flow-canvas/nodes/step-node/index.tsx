...
import { PieceSelector } from '@/app/builder/qadams-selector';
import { LoopIterationInput } from '@/app/builder/run-details/loop-iteration-input';
import { RightSideBarType } from '@/app/builder/types';
import { stepsHooks } from '@/features/qadams';
...
const ApStepCanvasNode = React.memo(
  ({ data: { step } }: NodeProps & Omit<ApStepNode, 'position'>) => {
    const [
      ...
      state.openedQadamSelectorStepNameOrAddButtonId === step.name,
    ...
    const handleStepClick = (
      ...
      if (step.type === FlowTriggerType.EMPTY) {
        setOpenedQadamSelectorStepNameOrAddButtonId(step.name);
      }
      if (preventDefault) {
        e.stopPropagation();
        e.preventDefault();
        ...
        >
          {!isDragging && (
            <PieceSelector
              ...
              stepToReplaceQadamDisplayName={stepMetadata?.displayName}
            >
              {isHorizontal ? (
                <div
                ...
                >
                  ...
                  <StepNodeDisplayName
                    ...
                    stepIndex={stepIndex}
                    isSkipped={isSkipped}
                    qadamDisplayName={stepMetadata?.displayName ?? ''}
                    stepName={step.name}
                  />
