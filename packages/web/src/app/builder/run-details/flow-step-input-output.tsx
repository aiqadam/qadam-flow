...
export const FlowStepInputOutput = () => {
  ...
  const stepInput =
    ...
    selectedStep.type === FlowTriggerType.PIECE
      ...
      : undefined;
  const pieceSchema =
    qadamModel?.actions[stepName ?? '']?.outputSchema ??
    qadamModel?.triggers[stepName ?? '']?.outputSchema ??
    null;
  const explanationContext: ErrorExplanationContext = {
    qadamName: stepPieceName,
    qadamVersion: stepPieceVersion,
    qadamDisplayName: qadamModel?.displayName,
    qadamAuthType: stepPropertiesSnapshotUtils.findAuthType(qadamModel),
    stepKind,
    stepName,
