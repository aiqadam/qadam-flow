...
import { qadamSelectorUtils, qadamsHooks } from '@/features/qadams';

import { DynamicPropertiesContext } from '../qadam-properties/dynamic-properties-context';
...
const isReturnResponseAndWaitForWebhook = (step: FlowAction) =>
  ...
  step.settings.qadamName === '@aiqadam/qadam-webhook' &&
...
const TriggerTestRunnerProvider = ({
  ...
  const isQadamTrigger = step.type === FlowTriggerType.PIECE;
  const qadamName = isQadamTrigger ? step.settings.qadamName : '';
  const qadamVersion = isQadamTrigger ? step.settings.qadamVersion : undefined;
  const triggerName = isQadamTrigger ? step.settings.triggerName : undefined;

  const { qadamModel, isLoading: isPieceLoading } = qadamsHooks.useQadam({
    name: qadamName,
    version: qadamVersion,
    enabled: isQadamTrigger && !!qadamName,
  });

  const trigger = triggerName ? qadamModel?.triggers?.[triggerName] : undefined;
  const mockData = trigger?.sampleData;

  const testType: TestType | null =
    trigger && triggerName && qadamName
      ? triggerEventUtils.getTestType({ triggerName, qadamName, trigger })
      : null;

  const isManualTrigger =
    qadamName && triggerName
      ? qadamSelectorUtils.isManualTrigger({ qadamName, triggerName })
      : false;
...
type TriggerTestRunnerContextValue = {
  step: FlowTrigger;
  qadamModel: ReturnType<typeof qadamsHooks.useQadam>['qadamModel'];
