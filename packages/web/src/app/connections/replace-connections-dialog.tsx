import { AppConnectionScope, PopulatedFlow } from '@aiqadam/shared';
...
const ReplaceConnectionsDialog = ({
  ...
  const { qadams, isLoading: qadamsLoading } = qadamsHooks.useQadams({});
  ...
  const form = useForm<FormData>({
    ...
    resolver: (values) => {
      ...
      if (!values.qadamName) {
        errors.qadamName = {
          type: 'required',
          message: t('Please select a qadam'),
        };
      }
  ...
  const selectedQadam = form.watch('qadamName');

  const connectionPieceNames = new Set(
    connections?.data.map((conn) => conn.qadamName),
  );
  ...
  const piecesOptions =
    qadams
      ?.filter(
        (piece) =>
          piece.name !== '@aiqadam/qadam-mcp' &&
          piece.name !== '@aiqadam/qadam-webhook' &&
  ...
  const filteredConnections =
    connections?.data.filter((conn) => conn.qadamName === selectedQadam) ?? [];
                ...
                render={({ field }) => (
                  <div className="flex flex-col gap-2">
                    ...
                    <SearchableSelect
                      ...
                      placeholder={t('Select a qadam')}
                      loading={qadamsLoading}
                      valuesRendering={(value) => {
                        const piece = qadams?.find((p) => p.name === value);
                        return (
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <SearchableSelect
                          ...
                          valuesRendering={(value) => {
                            ...
                            return (
                              <div className="flex gap-2 items-center">
                                <QadamIconWithQadamName
                                  qadamName={conn!.qadamName}
                                  size="xs"
                                  border={false}
                                />
