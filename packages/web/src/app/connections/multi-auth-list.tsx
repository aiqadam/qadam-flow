...
export function MutliAuthList({
  ...
  const authItems: RadioGroupListItem<AuthListItem>[] = qadamAuth.flatMap(
    (auth) => {
...
const getDisplayName = (auth: QadamAuthProperty): string => {
  if (
...
export type AuthListItem =
  | {
      authProperty: Exclude<QadamAuthProperty, OAuth2Property<any>>;
      grantType: null;
      oauth2App: null;
    }
  | {
...
type MutliAuthListProps = {
  qadamAuth: QadamAuthProperty[];
  setSelectedItem: (auth: AuthListItem) => void;
  confirmSelectedItem: () => void;
  piecesOAuth2AppsMap: QadamsOAuth2AppsMap;
