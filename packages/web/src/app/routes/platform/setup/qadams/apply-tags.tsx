...
import { CreateTagDialog } from '@/app/routes/platform/setup/qadams/create-tag-dialog';
...
type ApplyTagsProps = {
  selectedQadams: QadamMetadataModelSummary[];
  onApplyTags: () => void;
};

const ApplyTags = ({ selectedQadams, onApplyTags }: ApplyTagsProps) => {
    ...
    setSelectedTags(
      new Set(
        tags
          .map((tag) => tag.name)
          .filter((tag) =>
            selectedQadams.every((piece) => piece.tags?.includes(tag)),
          ),
      ),
    );
  }, [selectedQadams, tags]);
    ...
    >
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          disabled={selectedQadams.length === 0}
        >
          {t('Apply Tags')}
        </Button>
      </PopoverTrigger>
                ...
                <CommandGroup>
                  {tagOptions.map((option) => {
                    ...
                    const isIndeterminate =
                      selectedQadams.some((piece) =>
                        piece.tags?.includes(option.value),
                      ) &&
                      !selectedQadams.every((piece) =>
                        piece.tags?.includes(option.value),
                      ) &&
            ...
            <CommandGroup>
              <CommandItem
                ...
                onSelect={() => {
                  ...
                  applyTags({
                    qadamsName: selectedQadams.map((piece) => piece.name),
