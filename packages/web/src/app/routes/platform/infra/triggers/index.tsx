import { TriggerStatusReport } from '@aiqadam/shared';
...
import { QadamDisplayName, QadamIconWithQadamName } from '@/features/qadams';
...
export default function TriggerHealthPage() {
  ...
  const triggerHealthData: TriggerHealthRow[] = isLoading
    ? []
    : Object.entries(report?.pieces ?? {}).map(([qadamName, pieceData]) => {
  ...
  const columns = [
    {
      ...
      size: 220,
      header: ({ column }: any) => (
        <DataTableColumnHeader column={column} title="Qadam" icon={Puzzle} />
      ),
      cell: ({ row }: any) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <QadamIconWithQadamName
              qadamName={row.original.id}
              showTooltip={false}
              size="md"
            />
            <div className="flex flex-col">
              <div className="font-medium flex items-center gap-2">
                <QadamDisplayName qadamName={row.original.id} />
                <Tooltip>
