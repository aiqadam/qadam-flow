...
import { QadamIconWithQadamName } from '@/features/qadams/components/qadam-icon-from-name';
import { authenticationSession } from '@/lib/authentication-session';

import { normalizeQadamName } from '../lib/message-parsers';
...
export function ConnectionsRequiredCard({
  ...
  const activePieceName = activeConnection
    ? normalizeQadamName(activeConnection.piece)
    : null;
  ...
  useEffect(() => {
    ...
    void Promise.all(
      connections.map(async (conn) => {
        const qadamName = normalizeQadamName(conn.piece);
...
function ConnectionRow({
  ...
  const qadamName = normalizeQadamName(connection.piece);
  const { isLoading } = qadamsHooks.useQadam({ name: qadamName });
  const isReconnect =
