...
function buildOAuth2ValueSchema(
  ...
  connectionType: AppConnectionType.OAUTH2 | AppConnectionType.PLATFORM_OAUTH2,
) {
...
function buildCustomAuthValueSchema(auth: QadamAuthProperty) {
...
export const formUtils = {
  ...
  buildPieceSchema: (
    ...
    switch (type) {
      ...
      case FlowActionType.PIECE: {
        return QadamActionSchema.omit({ settings: true }).extend(
          z.object({
            settings: QadamActionSettings.omit({
              input: true,
              actionName: true,
            }).extend(
              z.object({
      ...
      case FlowTriggerType.PIECE: {
        return QadamTrigger.omit({ settings: true }).extend(
          z.object({
            settings: QadamTriggerSettings.omit({
              input: true,
              triggerName: true,
            }).extend(
              z.object({
