...
type TabType = {
  value: QadamSelectorTabType;
  name: string;
  icon: React.ReactNode;
};

export const QadamSelectorTabs = ({ tabs }: { tabs: TabType[] }) => {
  const { selectedTab, setSelectedTab } = useQadamSelectorTabs();
  return (
    <Tabs
      value={selectedTab}
      onValueChange={(value) => setSelectedTab(value as QadamSelectorTabType)}
      className="w-full"
    >
      <TabsList
