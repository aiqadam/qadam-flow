...
import { qadamsApi } from '@/features/qadams/api/qadams-api';
import { qadamSelectorUtils } from '@/features/qadams/utils/qadam-selector-utils';
import { stepUtils } from '@/features/qadams/utils/step-utils';
  ...
  useCreateMcpFlow: () => {
    ...
    return useMutation({
      mutationFn: async () => {
        ...
        const mcpPiece = await qadamsApi.get({
          name: '@aiqadam/qadam-mcp',
        });
        const trigger = mcpPiece.triggers['mcp_tool'];
        if (!trigger) {
          throw new Error('MCP trigger not found');
        }
        const stepData = qadamSelectorUtils.getDefaultStepValues({
          stepName: 'trigger',
          qadamSelectorItem: {
            actionOrTrigger: trigger,
            type: FlowTriggerType.PIECE,
            qadamMetadata: stepUtils.mapPieceToMetadata({
              piece: mcpPiece,
              type: 'trigger',
            }),
          },
