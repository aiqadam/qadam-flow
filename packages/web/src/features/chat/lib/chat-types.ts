...
function normalizeQadamName(name: string): string {
...
function extractQadamNames(
  input: Record<string, unknown> | undefined,
): string[] {
  if (!input) return [];
  const names: string[] = [];
  if (typeof input.qadamName === 'string') {
    names.push(input.qadamName);
  }
  if (Array.isArray(input.qadamNames)) {
    for (const n of input.qadamNames) {
  ...
  if (
    ...
    names.push(input.settings.qadamName);
  }
  return names.map(normalizeQadamName);
}
