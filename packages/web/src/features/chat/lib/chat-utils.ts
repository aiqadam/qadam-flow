...
function stripQadamPrefix(name: string): string {
  return name.replace(/^@aiqadam\/qadam-/, '');
}

function humanizeQadamName(raw: string): string {
  return formatUtils.convertEnumToHumanReadable(
    stripQadamPrefix(raw).replace(/-/g, '_'),
  );
}
...
function extractToolContext({
  ...
  if (typeof input.qadamName === 'string') {
    parts.push(humanizeQadamName(input.qadamName));
  }
  ...
  if (
    ...
    typeof input.settings.qadamName === 'string' &&
    parts.length === 0
  ) {
    parts.push(humanizeQadamName(input.settings.qadamName));
  }
