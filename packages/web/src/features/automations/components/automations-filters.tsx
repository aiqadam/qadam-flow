...
type AutomationsFiltersProps = {
  ...
  pieces: QadamMetadataModelSummary[] | undefined;
...
export const AutomationsFilters = ({
  ...
  const connectionOptions = (connections || []).map((connection) => {
    const qadamIcon = pieces?.find(
      (p) => p.name === connection.qadamName,
    )?.logoUrl;
    return {
      value: connection.externalId,
      label: connection.displayName,
      icon: qadamIcon ? (
        <img src={qadamIcon} alt="" className="h-4 w-4 object-contain" />
      ) : undefined,
    };
  });
