import { Tag } from '@aiqadam/shared';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { t } from 'i18next';
import { toast } from 'sonner';

import { piecesTagsApi } from '../api/qadams-tags';
...
export const piecesTagMutations = {
  ...
  useApplyTags: ({ onSuccess }: { onSuccess: () => void }) => {
    return useMutation({
      mutationFn: async ({ qadamsName, tags }: ApplyTagsParams) => {
        await piecesTagsApi.tagPieces({ qadamsName, tags });
      },
      onSuccess: () => {
