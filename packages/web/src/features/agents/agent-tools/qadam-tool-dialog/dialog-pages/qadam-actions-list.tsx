...
interface QadamActionsDialogProps {
  tools: AgentTool[];
}

export const QadamActionsList: React.FC<QadamActionsDialogProps> = ({
  tools,
}) => {
  const [searchQuery, setSearchQuery] = useState('');
  const [debouncedQuery] = useDebounce(searchQuery, 200);
  const { handleActionSelect, selectedQadam } = useQadamToolsDialogStore();
  ...
  const fuse = useMemo(() => {
    if (isNil(selectedQadam) || isNil(selectedQadam.suggestedActions))
      return null;

    return new Fuse(selectedQadam.suggestedActions, {
      keys: [
  ...
  }, [selectedQadam?.suggestedActions]);

  const filteredActions = useMemo(() => {
    if (!debouncedQuery.trim() || isNil(fuse))
      return selectedQadam?.suggestedActions || [];

    return fuse.search(debouncedQuery).map((r) => r.item);
  }, [debouncedQuery, fuse, selectedQadam?.suggestedActions]);

  if (isNil(selectedQadam)) {
