...
export const QadamsList: React.FC<PiecesContentProps> = ({
  isPiecesLoading,
  qadamMetadata,
}) => {
  const { searchQuery, setSearchQuery, handleQadamSelect } =
    useQadamToolsDialogStore();
  const isEmpty = !isPiecesLoading && qadamMetadata.length === 0;
    ...
    <div className="flex flex-col h-full">
      ...
      <ScrollArea className="flex-1 min-h-0 px-4 py-2">
        {isPiecesLoading ? (
          ...
          <div className="grid grid-cols-3 gap-4">
            {qadamMetadata.map((piece, index) => (
              <div
                key={index}
                onClick={() => handleQadamSelect(piece)}
