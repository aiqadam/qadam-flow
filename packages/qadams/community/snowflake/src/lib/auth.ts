...
export const snowflakeOAuth2Auth = QadamAuth.OAuth2({
  displayName: 'OAuth',
...
CREATE SECURITY INTEGRATION "activepieces"
  ...
  OAUTH_REDIRECT_URI = 'https://flow.aiqadam.org/redirect'
  ENABLED = TRUE
...
export const snowflakeCustomAuth = QadamAuth.CustomAuth({
...
**3. Paste the private key** — copy the full contents of \`rsa_key.p8\` (including the \`-----BEGIN PRIVATE KEY-----\` header and footer) into the **Private Key** field below.`,
  props: {
    ...
    privateKey: QadamAuth.SecretText({
    ...
    privateKeyPassphrase: QadamAuth.SecretText({
