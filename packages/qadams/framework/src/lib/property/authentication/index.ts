...
export const QadamAuthProperty = z.union([
  BasicAuthProperty,
  CustomAuthProperty,
  OAuth2Property,
  SecretTextProperty,
...
export type QadamAuthProperty = BasicAuthProperty | CustomAuthProperty<any> | OAuth2Property<any> | SecretTextProperty<boolean>;
...
export type ExtractQadamAuthPropertyTypeForMethods<T extends QadamAuthProperty | QadamAuthProperty[] | undefined> = T extends QadamAuthProperty[] ? T[number] : T extends undefined ? undefined : T;

export function getAuthPropertyForValue({ authValueType, qadamAuth }: GetAuthPropertyForValue) {
  if (!Array.isArray(qadamAuth) || isNil(qadamAuth)) {
    return qadamAuth;
  }

  return qadamAuth.find(auth => authConnectionTypeToPropertyType[authValueType] === auth.type) ?? qadamAuth.at(0);
}
