...
import { ExecutionType, FlowStatus, FlowTriggerType, PARENT_RUN_LOCALE_HEADER, PopulatedFlow } from '@aiqadam/shared'
...
function buildContext(params: { locale: (() => Promise<string | null>) | undefined }) {
    return {
        ...
        run: {
            ...
            // this test exists to prove call-flow.ts guards against.
            ...(params.locale === undefined ? {} : { locale: params.locale }),
        },
    } as unknown as Parameters<typeof import('../src/lib/actions/call-flow').callFlow.run>[0]
}

describe('callFlow (queue mode) forwards the parent run locale header', () => {
    beforeEach(() => {
        vi.restoreAllMocks()
        sendRequest.mockReset()
        sendRequest.mockResolvedValue({ body: { status: 'success', data: {} } })
    })

    it('sends the header when the run has a resolved locale', async () => {
        const { callFlow } = await import('../src/lib/actions/call-flow')
        const context = buildContext({ locale: async () => 'ru' })

        await callFlow.run(context)
    ...
    it('omits the header, without throwing, on an engine whose RunContext has no locale hook at all', async () => {
        ...
        await expect(callFlow.run(context)).resolves.toBeDefined()
